...
void TriggerDynamicPointLight(const Vector3& pos, const Color& color, float falloff)
{
	g_Renderer.AddDynamicPointLight(pos, falloff , color);
}

void TriggerDynamicSpotLight(const Vector3& pos, const Vector3& dir, const Color& color, float radius, float falloff, float distance)
{
	g_Renderer.AddDynamicSpotLight(pos, dir, radius, falloff, distance, color);
}

// Deprecated. Use above version instead.
void TriggerDynamicLight(int x, int y, int z, short falloff, byte r, byte g, byte b)
{
	g_Renderer.AddDynamicPointLight(Vector3(x, y, z), (float)(falloff * UCHAR_MAX), Color(r / (float)UCHAR_MAX, g / (float)UCHAR_MAX, b / (float)UCHAR_MAX));
}
