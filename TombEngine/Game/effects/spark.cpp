...
namespace TEN::Effects::Spark
{
	...
	void TriggerFrictionSpark(const GameVector& pos, const EulerAngles& angle, float length, int count)
	{
		for (int i = 0; i < count; i++)
		{
			...
			s = {};
			s.age = 0;
			s.life = Random::GenerateFloat(8, 15);
			s.friction = 0.1f;
			s.gravity = 0.0f;
			s.height = length;
			s.width = Random::GenerateFloat(16.0f, 32.0f);
			s.room = pos.RoomNumber;
			s.pos = Vector3(pos.x + Random::GenerateFloat(-16, 16), pos.y + Random::GenerateFloat(-16, 16), pos.z + Random::GenerateFloat(-16, 16));
	...
	void TriggerElectricSpark(const GameVector& pos, const EulerAngles& angle, int count)
	{
		for (int i = 0; i < count; i++)
		{
			...
			s.friction = 1.0f;
			s.gravity = 2.0f;
			s.height = Random::GenerateFloat(64.0f, 256.0f);
			s.width = Random::GenerateFloat(8.0f, 16.0f);
			s.room = pos.RoomNumber;
			s.pos = Vector3(pos.x + Random::GenerateFloat(-16, 16), pos.y + Random::GenerateFloat(-16, 16), pos.z + Random::GenerateFloat(-16, 16));
			float ang = TO_RAD(angle.y);
			float vAng = -TO_RAD(angle.x);
