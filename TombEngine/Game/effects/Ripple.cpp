...
namespace TEN::Effects::Ripple
{
	...
	void SpawnRipple(const Vector3& pos, int roomNumber, float size, int flags, const Vector3& normal)
	{
		...
		constexpr auto COLOR_WHITE			  = Vector4(1.0f, 1.0f, 1.0f, Ripple::OPACITY_MAX);

		auto& ripple = GetNewEffect(Ripples, COUNT_MAX);
	...
	void UpdateRipples()
	{
		...
		constexpr auto SIZE_STEP_LARGE = RIPPLE_SIZE_MAX / 128;
		constexpr auto SIZE_STEP_SMALL = SIZE_STEP_LARGE / 2;
		...
		for (auto& ripple : Ripples)
		{
			...
			if (ripple.Life >= lifeFullOpacity)
			{
				...
				ripple.Color.w = Lerp(0.0f, Ripple::OPACITY_MAX, alpha);
			}
			else if (ripple.Life <= lifeStartFading)
			{
				float alpha = 1.0f - (ripple.Life / lifeStartFading);
				ripple.Color.w = Lerp(Ripple::OPACITY_MAX, 0.0f, alpha);
			}
