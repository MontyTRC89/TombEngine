...
namespace TEN::Effects::Lightning
{
	...
	void TriggerLightning(Vector3i* origin, Vector3i* target, unsigned char amplitude, unsigned char r, unsigned char g, unsigned char b, unsigned char life, char flags, char width, char segments)
	{
		LIGHTNING_INFO arc;

		arc.pos1 = *origin;
		arc.pos2.x = (target->x + 3 * origin->x) >> 2;
		arc.pos2.y = (target->y + 3 * origin->y) >> 2;
		arc.pos2.z = (target->z + 3 * origin->z) >> 2;
		arc.pos3.x = (origin->x + 3 * target->x) >> 2;
		arc.pos3.y = (origin->y + 3 * target->y) >> 2;
		arc.pos3.z = (origin->z + 3 * target->z) >> 2;
		arc.pos4 = *target;
		arc.flags = flags;
