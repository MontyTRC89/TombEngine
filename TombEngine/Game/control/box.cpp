...
bool MoveCreature3DPos(PHD_3DPOS* origin, PHD_3DPOS* target, int velocity, float angleDif, float angleAdd)
{
	...
	if (angleDif <= angleAdd)
	{
		if (angleDif >= -angleAdd)
			...
			origin->Orientation.y = origin->Orientation.y - angleAdd;
	}
	else
		origin->Orientation.y = origin->Orientation.y + angleAdd;
...
short CreatureEffect2(ItemInfo* item, BiteInfo bite, short velocity, float angle, std::function<CreatureEffectFunction> func)
{
...
void CreatureJoint(ItemInfo* item, short joint, float required)
{
	...
	else if (creature->JointRotation[joint] < Angle::DegToRad(-70.0f))
		creature->JointRotation[joint] = Angle::DegToRad(-70.0f);
}
...
void CreatureTilt(ItemInfo* item, float angle)
{
	...
	else if (angle > Angle::DegToRad(3.0f))
		angle = Angle::DegToRad(3.0f);

	float theAngle = Angle::DegToRad(-3.0f);

	float absRot = abs(item->Pose.Orientation.z);
...
float CreatureTurn(ItemInfo* item, float maxTurn)
{
	...
	int range = item->Animation.Velocity.z * (Angle::DegToRad(90.0f) / maxTurn);
...
int CreatureAnimation(short itemNumber, float angle, float tilt)
{
	...
	if (LOT->Fly != NO_FLYING && item->HitPoints > 0)
	{
		...
		angle = item->Animation.Velocity.z ? atan2(item->Animation.Velocity.z, -dy) : Angle::DegToRad(0.0f);
...
void CreatureAIInfo(ItemInfo* item, AI_INFO* AI)
{
	...
	if (enemy == LaraItem)
	{
		vector.x = enemy->Pose.Position.x + enemy->Animation.Velocity.z * PREDICTIVE_SCALE_FACTOR * sin(Lara.Control.MoveAngle) - item->Pose.Position.x - object->pivotLength * sin(item->Pose.Orientation.y);
		vector.z = enemy->Pose.Position.z + enemy->Animation.Velocity.z * PREDICTIVE_SCALE_FACTOR * cos(Lara.Control.MoveAngle) - item->Pose.Position.z - object->pivotLength * cos(item->Pose.Orientation.y);
	}
	else
	{
		vector.x = enemy->Pose.Position.x + enemy->Animation.Velocity.z * PREDICTIVE_SCALE_FACTOR * sin(enemy->Pose.Orientation.y) - item->Pose.Position.x - object->pivotLength * sin(item->Pose.Orientation.y);
		vector.z = enemy->Pose.Position.z + enemy->Animation.Velocity.z * PREDICTIVE_SCALE_FACTOR * cos(enemy->Pose.Orientation.y) - item->Pose.Position.z - object->pivotLength * cos(item->Pose.Orientation.y);
	}
	...
	if (vector.x > vector.z)
		AI->xAngle = atan2(vector.x + (vector.z >> 1), vector.y);
	else
		AI->xAngle = atan2(vector.z + (vector.x >> 1), vector.y);
...
void AdjustStopperFlag(ItemInfo* item, int direction, bool set)
{
	...
	z = item->Pose.Position.z + SECTOR(1) * cos(direction);
