...
void GenericSphereBoxCollision(short itemNumber, ItemInfo* laraItem, CollisionInfo* coll)
{
	...
	if (item->Status != ITEM_INVISIBLE)
	{
		if (TestBoundsCollide(item, laraItem, coll->Setup.Radius))
		{
			...
			if (collidedBits)
			{
				auto prevYOrient = item->Pose.Orientation.y;
...
void TestForObjectOnLedge(ItemInfo* item, CollisionInfo* coll)
{
	...
	for (int i = 0; i < 3; i++)
	{
		auto sinForwardAngle = (i != 1) ? (sin(coll->Setup.ForwardAngle + Angle::DegToRad((i * 90.0f) - 90.0f))) : 0;
		auto cosForwardAngle = (i != 1) ? (cos(coll->Setup.ForwardAngle + Angle::DegToRad((i * 90.0f) - 90.0f))) : 0;
		...
		auto mxR = Matrix::CreateFromYawPitchRoll(coll->Setup.ForwardAngle, 0.0f, 0.0f);
...
bool MoveLaraPosition(Vector3Int* offset, ItemInfo* item, ItemInfo* laraItem)
{
	...
	if (!Objects[item->ObjectNumber].isPickup)
		return Move3DPosTo3DPos(&laraItem->Pose, &target, LARA_ALIGN_VELOCITY, Angle::DegToRad(2.0f));
	else
	{
...
bool Move3DPosTo3DPos(PHD_3DPOS* fromPose, PHD_3DPOS* toPose, int velocity, float angleAdd)
{
	...
	if (!Lara.Control.IsMoving)
	{
		...
		{
			float angle = GetOrientTowardPoint(toPose->Position.ToVector3(), fromPose->Position.ToVector3()).y;
	...
	auto deltaAngle = toPose->Orientation.x - fromPose->Orientation.x;
...
bool ItemPushItem(ItemInfo* item, ItemInfo* item2, CollisionInfo* coll, bool spasmEnabled, char bigPush)
{
	...
	coll->Setup.ForwardAngle = atan2(item2->Pose.Position.x - coll->Setup.OldPosition.x, item2->Pose.Position.z - coll->Setup.OldPosition.z); // Correct order.
...
bool CollideSolidBounds(ItemInfo* item, BOUNDING_BOX* box, PHD_3DPOS pose, CollisionInfo* coll)
{
	...
	auto sinY = sin(pose.Orientation.y);
	auto cosY = cos(pose.Orientation.y);
	...
	sinY = sin(-pose.Orientation.y);
	cosY = cos(-pose.Orientation.y);
			...
			else if (pointProbe.FloorTilt.x > 0 && ((abs(pointProbe.FloorTilt.x)) - (abs(pointProbe.FloorTilt.y)) >= 2))
			{
				...
				else
				{
					if (item->Animation.Velocity.z < 32)
					{
						...
						{
							item->Pose.Orientation.y -= Angle::DegToRad(22.5f)	;
							if (item->Pose.Orientation.y < Angle::DegToRad(270.0f))
			...
			else if (pointProbe.FloorTilt.y < 0 && ((abs(pointProbe.FloorTilt.y)) - (abs(pointProbe.FloorTilt.x)) >= 2))
			{
				...
				else
				{
					if (item->Animation.Velocity.z < 32)
					{
						...
						{
							item->Pose.Orientation.y -= Angle::DegToRad(22.5f);
							if (item->Pose.Orientation.y > Angle::DegToRad(337.5))
								item->Pose.Orientation.y = 0.0f;
						}
						else if (item->Pose.Orientation.y >= Angle::DegToRad(180.0f))
						{
							item->Pose.Orientation.y += Angle::DegToRad(22.5f);
							if (item->Pose.Orientation.y < Angle::DegToRad(22.5f))
								item->Pose.Orientation.y = 0.0f;
						}
					}
			...
			else if (pointProbe.FloorTilt.x < 0 && pointProbe.FloorTilt.y < 0)	// Hit angle = 0x2000
			{
				...
				else
				{
					if (item->Animation.Velocity.z < 32)
					{
						...
						{
							item->Pose.Orientation.y -= Angle::DegToRad(22.5f);
							if (item->Pose.Orientation.y < Angle::DegToRad(45.0f))
								item->Pose.Orientation.y = Angle::DegToRad(45.0f);
						}
						else if (item->Pose.Orientation.y != Angle::DegToRad(45.0f))
						{
							item->Pose.Orientation.y += Angle::DegToRad(22.5f);
							if (item->Pose.Orientation.y > Angle::DegToRad(45.0f))
								item->Pose.Orientation.y = Angle::DegToRad(45.0f);
			...
			else if (pointProbe.FloorTilt.x < 0 && pointProbe.FloorTilt.y > 0)
			{
				...
				else
				{
					if (item->Animation.Velocity.z < 32)
					{
						...
						{
							item->Pose.Orientation.y -= Angle::DegToRad(22.5f);
							if (item->Pose.Orientation.y < Angle::DegToRad(135.0f))
								item->Pose.Orientation.y = Angle::DegToRad(135.0f);
						}
						else if (item->Pose.Orientation.y != Angle::DegToRad(135.0f))
						{
							item->Pose.Orientation.y += Angle::DegToRad(22.5f);
							if (item->Pose.Orientation.y > Angle::DegToRad(135.0f))
								item->Pose.Orientation.y = Angle::DegToRad(135.0f);
			...
			else if (pointProbe.FloorTilt.x > 0 && pointProbe.FloorTilt.y > 0)
			{
				...
				else
				{
					if (item->Animation.Velocity.z < 32)
					{
						...
						{
							item->Pose.Orientation.y -= Angle::DegToRad(22.5f);
							if (item->Pose.Orientation.y < Angle::DegToRad(225.5f))
								item->Pose.Orientation.y = Angle::DegToRad(225.5f);
						}
						else if (item->Pose.Orientation.y != Angle::DegToRad(225.5f))
						{
							item->Pose.Orientation.y += Angle::DegToRad(22.5f);
							if (item->Pose.Orientation.y > Angle::DegToRad(225.5f))
								item->Pose.Orientation.y = Angle::DegToRad(225.5f);
			...
			else if (pointProbe.FloorTilt.x > 0 && pointProbe.FloorTilt.y < 0)
			{
				...
				else
				{
					if (item->Animation.Velocity.z < 32)
					{
						...
						{
							item->Pose.Orientation.y -= Angle::DegToRad(22.5f);
							if (item->Pose.Orientation.y < Angle::DegToRad(315.0f))
								item->Pose.Orientation.y = Angle::DegToRad(315.0f);
						}
						else if (item->Pose.Orientation.y != Angle::DegToRad(315.0f))
						{
							item->Pose.Orientation.y += Angle::DegToRad(22.5f);
							if (item->Pose.Orientation.y > Angle::DegToRad(315.0f))
								item->Pose.Orientation.y = Angle::DegToRad(315.0f);
...
void CreatureCollision(short itemNumber, ItemInfo* laraItem, CollisionInfo* coll)
{
	...
	else if (doPlayerCollision && coll->Setup.EnableSpasm)
	{
		...
		if (frame->boundingBox.Height() > CLICK(1))
		{
			...
			int angle = (laraItem->Pose.Orientation.y - phd_atan(z - cosY * rx - sinY * rz, x - cosY * rx + sinY * rz) - Angle::DegToRad(135.0f)) / Angle::DegToRad(90.0f);
