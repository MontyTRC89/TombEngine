...
LedgeData GetNearestLedgeData(const ItemInfo& item, const CollisionInfo& coll)
{
	...
	for (int h = 0; h < 2; h++)
	{
		...
		for (int p = 0; p < 3; p++)
		{
			...
			int floorHeight   = GetFloorHeight(RoomVector(block->RoomNumber, y), ffpX, ffpZ).value_or(NO_HEIGHT);
			int ceilingHeight = GetCeilingHeight(RoomVector(block->RoomNumber, y), ffpX, ffpZ).value_or(NO_HEIGHT);
			...
			else
			{
				...
				float splitAngle = (useCeilingLedge ? TO_RAD(block->CeilingSurface.SplitAngle) : TO_RAD(block->FloorSurface.SplitAngle));
