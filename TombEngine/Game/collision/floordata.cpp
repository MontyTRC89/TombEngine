...
std::vector<std::vector<Vector3>> FloorInfo::GetSurfaceVertexGroups(int x, int z, bool isFloor)
{
	...
	if (IsSurfaceSplit(isFloor))
	{
		...
		for (int i = 0; i < QUAD_VERTEX_COUNT; i++)
		{
			...
			if (FloorSurface.SplitAngle == SectorSurfaceData::SPLIT_ANGLE_0)
			{
				if (i == 0)
				{
					x += 1;
...
int FloorInfo::GetSurfaceHeight(int triID, int x, int z, bool isFloor) const
{
	// Get triangle.
	const auto& surface = isFloor ? FloorSurface : CeilingSurface;
...
{
	int triID = GetSurfaceTriangleID(x, z, isFloor);
	return GetSurfaceHeight(triID, x, z, isFloor);
}
