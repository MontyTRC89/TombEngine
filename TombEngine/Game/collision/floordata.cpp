...
namespace TEN::Collision::Floordata
{
	...
	std::optional<RoomVector> GetBottomRoom(RoomVector location, const Vector3i& pos)
	{
		...
		if (floor->IsWall(pos.x, pos.z))
		{
			...
			location.Height = floor->GetSurfaceHeight(pos.x, pos.z, false);
		}

		auto floorHeight = floor->GetSurfaceHeight(Vector3i(pos.x, location.Height, pos.z), true);
		auto ceilingHeight = floor->GetSurfaceHeight(Vector3i(pos.x, location.Height, pos.z), false);

		location.Height = std::clamp(location.Height, std::min(ceilingHeight, floorHeight), std::max(ceilingHeight, floorHeight));

		if (floor->GetInsideBridgeItemNumber(Vector3i(pos.x, location.Height, pos.z), location.Height == ceilingHeight, location.Height == floorHeight) >= 0)
		{
			const auto height = GetBottomHeight(*floor, Vector3i(pos.x, location.Height, pos.z), &location.RoomNumber, &floor);
			if (!height)
				return std::nullopt;
		...
		floorHeight = floor->GetSurfaceHeight(Vector3i(pos.x, location.Height, pos.z), true);
		ceilingHeight = floor->GetSurfaceHeight(Vector3i(pos.x, location.Height, pos.z), false);

		if (pos.y < ceilingHeight && floor->GetRoomNumberAbove(Vector3i(pos.x, location.Height, pos.z)))
			return std::nullopt;
		...
		auto roomBelow = floor->GetRoomNumberBelow(Vector3i(pos.x, location.Height, pos.z));
		while (roomBelow)
		{
			...
			roomBelow = floor->GetRoomNumberBelow(Vector3i(pos.x, location.Height, pos.z));
		}
	...
	std::optional<RoomVector> GetTopRoom(RoomVector location, const Vector3i& pos)
	{
		...
		if (floor->IsWall(pos.x, pos.z))
		{
			...
			location.Height = floor->GetSurfaceHeight(pos.x, pos.z, true);
		}
		...
		if (floor->GetInsideBridgeItemNumber(Vector3i(pos.x, location.Height, pos.z), location.Height == ceilingHeight, location.Height == floorHeight) >= 0)
		{
			const auto height = GetTopHeight(*floor, Vector3i(pos.x, location.Height, pos.z), &location.RoomNumber, &floor);
			if (!height)
				return std::nullopt;
		...
		if (pos.y > floorHeight && floor->GetRoomNumberBelow(Vector3i(pos.x, location.Height, pos.z)))
			return std::nullopt;
		...
		auto roomAbove = floor->GetRoomNumberAbove(Vector3i(pos.x, location.Height, pos.z));
		while (roomAbove)
		{
			...
			roomAbove = floor->GetRoomNumberAbove(Vector3i(pos.x, location.Height, pos.z));
		}
