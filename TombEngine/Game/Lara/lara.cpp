...
static void InitAttractors(ItemInfo& item)
{
	...
	player.Context.DebugAttractor0 = Attractor(AttractorType::Edge, points, item.RoomNumber);
	player.Context.DebugAttractor1 = Attractor(AttractorType::Edge, points, item.RoomNumber);
	player.Context.DebugAttractor2 = Attractor(AttractorType::Edge, points, item.RoomNumber);
}
...
static void SpawnAttractorPentagon(ItemInfo& item, bool isOuter)
{
	...
	player.Context.DebugAttractor2 = attrac;
}
...
static void SetDebugAttractors(ItemInfo& item)
{
	...
	if (KeyMap[OIS::KeyCode::KC_Q])
	{
		...
		auto pos1 = player.Context.DebugAttractor0.GetPoints().empty() ? Vector3::Zero : player.Context.DebugAttractor0.GetPoints()[1];
		player.Context.DebugAttractor0 = Attractor(AttractorType::Edge, { pos0, pos1 }, item.RoomNumber);
	}
	if (KeyMap[OIS::KeyCode::KC_W])
	{
		auto pos0 = player.Context.DebugAttractor0.GetPoints().empty() ? Vector3::Zero : player.Context.DebugAttractor0.GetPoints()[0];
	...
	{
		auto pos0 = LaraItem->Pose.Position.ToVector3() + Vector3::Transform(Vector3(0.0f, -CLICK(5), LARA_RADIUS), rotMatrix);
		auto pos1 = player.Context.DebugAttractor1.GetPoints().empty() ? Vector3::Zero : player.Context.DebugAttractor1.GetPoints()[1];
		player.Context.DebugAttractor1 = Attractor(AttractorType::Edge, { pos0, pos1 }, item.RoomNumber);
	}
	if (KeyMap[OIS::KeyCode::KC_R])
	{
		auto pos0 = player.Context.DebugAttractor1.GetPoints().empty() ? Vector3::Zero : player.Context.DebugAttractor1.GetPoints()[0];
		auto pos1 = LaraItem->Pose.Position.ToVector3() + Vector3::Transform(Vector3(0.0f, -CLICK(5), LARA_RADIUS), rotMatrix);
		player.Context.DebugAttractor1 = Attractor(AttractorType::Edge, { pos0, pos1 }, item.RoomNumber);
	}
...
void HandleAttractorDebug(ItemInfo& item)
{
	...
	if (attrac.has_value())
		player.Context.DebugAttractor1 = GenerateSectorAttractor(pointColl).value();
