...
namespace TEN::Entities::Player
{
	...
	bool PlayerContext::TestSidestep(bool isGoingRight)
	{
			...
			TestEnvironment(ENV_FLAG_SWAMP, PlayerItemPtr))
		{
			Context::GroundMovementSetup contextSetup =
			{
				PlayerItemPtr->Pose.Orientation.y + (isGoingRight ? Angle::DegToRad(90.0f) : Angle::DegToRad(-90.0f)),
	...
	bool PlayerContext::TestDirectionalStandingJump(float relativeHeadingAngle)
	{
		// Check for swamp.
		if (TestEnvironment(ENV_FLAG_SWAMP, PlayerItemPtr))
			return false;
