...
void lara_as_jump_forward(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (TrInput & IN_LEFT)
	{
		...
		if (lara->Control.TurnRate.GetY() < -LARA_JUMP_TURN_MAX)
			lara->Control.TurnRate.SetY(-LARA_JUMP_TURN_MAX);
	}
...
void lara_as_reach(ItemInfo* item, CollisionInfo* coll)
{
	...
	Camera.targetAngle = Angle::DegToRad(85.0f);
	...
	{
		lara->Control.TurnRate.SetY(lara->Control.TurnRate.GetY() - LARA_TURN_RATE);
		if (lara->Control.TurnRate.GetY() < (-LARA_JUMP_TURN_MAX / 2))
			lara->Control.TurnRate.SetY(-LARA_JUMP_TURN_MAX / 2);
	}
	else if (TrInput & IN_RIGHT)
	{
		lara->Control.TurnRate.SetY(lara->Control.TurnRate.GetY() +  LARA_TURN_RATE);
...
void lara_col_jump_prepare(ItemInfo* item, CollisionInfo* coll)
{
	...
	case JumpDirection::Left:
		lara->Control.MoveAngle -= Angle::DegToRad(90.0f);
		break;
