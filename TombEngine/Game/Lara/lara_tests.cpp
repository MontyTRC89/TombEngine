...
bool TestLaraHang(ItemInfo* item, CollisionInfo* coll)
{
	...
	else // Normal case
	{
		...
		else // Death, incorrect ledge or ACTION release
		{
			...
			item->Pose.Position.y += GameBoundingBox(item).Y2 * (g_Configuration.EnableAutoClimb ? 2.5f : 1.8f);
...
bool TestLaraHangJump(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (!IsHeld(In::Action) && !g_Configuration.EnableAutoClimb)
		return false;
...
bool TestLaraValidHangPosition(ItemInfo* item, CollisionInfo* coll)
{
	...
	auto frontFloor = GetCollision(item, lara->Control.HeadingOrient.y, coll->Setup.Radius + CLICK(0.5f), -LARA_HEIGHT).Position.Floor;
...
{
	const auto& player = GetLaraInfo(item);
	return g_Configuration.EnableAutoClimb ? player.Control.ToggleClimb : IsHeld(In::Action);
}
