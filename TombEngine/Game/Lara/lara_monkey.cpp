...
void lara_as_monkey_idle(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (TrInput & IN_ACTION && lara->Control.CanMonkeySwing)
	{
		...
		if (TrInput & IN_FORWARD && lara->Context.CanMonkeyForward(item, coll))
		{
			item->Animation.TargetState = LS_MONKEY_FORWARD;
			return;
		}
		else if (TrInput & IN_BACK && lara->Context.CanMonkeyBack(item, coll))
		{
		...
		{
			if (lara->Context.CanMonkeyShimmyLeft(item, coll))
		...
		{
			if (lara->Context.CanMonkeyShimmyRight(item, coll))
