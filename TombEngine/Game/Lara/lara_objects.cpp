...
void lara_as_tightrope_fall(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (TestLastFrame(item))
	{
		...
		if (item->Animation.AnimNumber == LA_TIGHTROPE_FALL_LEFT)
			TranslateItem(item, coll->Setup.ForwardAngle - Angle::DegToRad(90.0f), CLICK(1), 0, 0);
...
void lara_as_pole_idle(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (TrInput & IN_ACTION)
	{
		if (item->Animation.ActiveState == LA_POLE_IDLE) // Hack.
		{
			if (TrInput & IN_LEFT)
			{
				lara->Control.TurnRate.SetY(lara->Control.TurnRate.GetY() + LARA_POLE_TURN_RATE);
				if (lara->Control.TurnRate.GetY() > LARA_POLE_TURN_MAX)
					lara->Control.TurnRate.SetY(LARA_POLE_TURN_MAX);
			}
			else if (TrInput & IN_RIGHT)
			{
				lara->Control.TurnRate.SetY(lara->Control.TurnRate.GetY() - LARA_POLE_TURN_RATE);
