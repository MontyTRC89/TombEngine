...
void lara_as_swimcheat(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (TrInput & IN_LEFT)
	{
		lara->Control.TurnRate.SetY(lara->Control.TurnRate.GetY() - Angle::DegToRad(3.4f));
		if (lara->Control.TurnRate.GetY() < Angle::DegToRad(-6.0f))
			lara->Control.TurnRate.SetY(Angle::DegToRad(-6.0f));
	}
