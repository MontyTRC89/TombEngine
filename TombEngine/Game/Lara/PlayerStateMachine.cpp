...
namespace TEN::Entities::Player
{
	...
	void InitializePlayerStateMachine()
	{
		...
		PlayerBehaviorStateRoutines[LS_HANDSTAND] = std::pair(lara_as_edge_hang_handstand, lara_default_col);
		PlayerBehaviorStateRoutines[LS_TREAD_WATER_VAULT] = std::pair(lara_as_water_tread_vault, lara_void_func);
		PlayerBehaviorStateRoutines[LS_WALL_CLIMB_IDLE] = std::pair(lara_as_wall_climb_idle, lara_col_wall_climb_idle);
		PlayerBehaviorStateRoutines[LS_WALL_CLIMB_UP] = std::pair(lara_as_wall_climb_up, lara_col_wall_climb_up);
		PlayerBehaviorStateRoutines[LS_WALL_CLIMB_LEFT] = std::pair(lara_as_wall_climb_left, lara_col_wall_climb_left);
		PlayerBehaviorStateRoutines[LS_WALL_CLIMB_STOP] = std::pair(lara_as_wall_climb_end, lara_col_wall_climb_end);
		PlayerBehaviorStateRoutines[LS_WALL_CLIMB_RIGHT] = std::pair(lara_as_wall_climb_right, lara_col_wall_climb_right);
		PlayerBehaviorStateRoutines[LS_WALL_CLIMB_DOWN] = std::pair(lara_as_wall_climb_down, lara_col_wall_climb_down);
		...
		PlayerBehaviorStateRoutines[LS_EDGE_HANG_SHIMMY_90_OUTER_LEFT] = std::pair(lara_as_edge_hang_shimmy_corner, lara_as_null);
		PlayerBehaviorStateRoutines[LS_EDGE_HANG_SHIMMY_90_OUTER_RIGHT] = std::pair(lara_as_edge_hang_shimmy_corner, lara_as_null);
		PlayerBehaviorStateRoutines[LS_EDGE_HANG_SHIMMY_90_INNER_LEFT] = std::pair(lara_as_edge_hang_shimmy_corner, lara_as_null);
		PlayerBehaviorStateRoutines[LS_EDGE_HANG_SHIMMY_90_INNER_RIGHT] = std::pair(lara_as_edge_hang_shimmy_corner, lara_as_null);
		...
		PlayerBehaviorStateRoutines[LS_WALL_CLIMB_DISMOUNT_LEFT] = std::pair(lara_as_wall_climb_dismount_left, lara_default_col);
		PlayerBehaviorStateRoutines[LS_WALL_CLIMB_DISMOUNT_RIGHT] = std::pair(lara_as_wall_climb_dismount_right, lara_default_col);
