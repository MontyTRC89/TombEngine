...
void lara_col_climb_down(ItemInfo* item, CollisionInfo* coll)
{
	...
	int frame = item->Animation.FrameNumber - GetAnimData(item->ObjectNumber, LA_WALL_CLIMB_DOWN).frameBase;
	int yShift = 0;
...
void lara_col_climb_up(ItemInfo* item, CollisionInfo* coll)
{
	...
	{
		int frame = item->Animation.FrameNumber - GetAnimData(item->ObjectNumber, LA_WALL_CLIMB_UP).frameBase;
		int yShift;
...
bool LaraCheckForLetGo(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (IsHeld(In::Action) && item->HitPoints > 0 || item->Animation.AnimNumber == LA_ONWATER_TO_WALL_CLIMB) // Can't let go on this anim
		return false;
