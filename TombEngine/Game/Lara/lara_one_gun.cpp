...
void AnimateShotgun(ItemInfo* laraItem, LaraWeaponType weaponType)
{
	...
	case WEAPON_STATE_RECOIL:
		if (item->Animation.FrameNumber == g_Level.Anims[item->Animation.AnimNumber].frameBase)
		{
			...
			if (lara->Control.WaterStatus != WaterStatus::Underwater && !running && !harpoonFired)
			{
				if ((TrInput & IN_ACTION) && (!lara->TargetEntity || lara->LeftArm.Locked))
				{
					...
					else if (weaponType == LaraWeaponType::HK)
					{
						...
						{
							SoundEffect(SFX_TEN_EXPLOSION1, &laraItem->Pose, 83888140);
							SoundEffect(SFX_TEN_HK_FIRE, &laraItem->Pose, 0);
						}
					}
					else
			...
			{
				StopSoundEffect(SFX_TEN_HK_FIRE);
				SoundEffect(SFX_TEN_HK_STOP, &laraItem->Pose, 0);
				//				HKFlag = 0;
			}
		}
		/*		else if (HKFlag)
				{
					...
					{
						SoundEffect(SFX_TEN_EXPLOSION1, &laraItem->pos, 83888140);
...
void FireShotgun(ItemInfo* laraItem)
{
	...
	if (fired)
	{
		...
		SoundEffect(SFX_TEN_EXPLOSION1, &laraItem->Pose, 20971524);
...
void FireCrossbow(ItemInfo* laraItem, PHD_3DPOS* pos)
{
	...
	if (itemNumber != NO_ITEM)
	{
		...
		SoundEffect(SFX_TEN_CROSSBOW_FIRE, 0, 0);
