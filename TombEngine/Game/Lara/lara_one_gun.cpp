...
void FireShotgun(ItemInfo* laraItem)
{
	...
	for (int i = 0; i < 6; i++)
	{
		auto wobbleArmOrient = EulerAngles(
			armOrient.x + value * Angle::ShrtToRad(GetRandomControl() - Angle::DegToShrt(90.0f)) / Angle::DegToRad(360.0f),
			armOrient.y + value * Angle::ShrtToRad(GetRandomControl() - Angle::DegToShrt(90.0f)) / Angle::DegToRad(360.0f),
			0.0f
		);
...
void FireHarpoon(ItemInfo* laraItem)
{
	...
	if (itemNumber != NO_ITEM)
	{
		...
		item->Animation.Velocity.z = HARPOON_VELOCITY * cos(item->Pose.Orientation.x);
		item->Animation.Velocity.y = -HARPOON_VELOCITY * sin(item->Pose.Orientation.x);
...
void HarpoonBoltControl(short itemNumber)
{
	...
	if (item->HitPoints == HARPOON_TIME)
	{
		...
		else
		{
			...
			item->Animation.Velocity.y = -HARPOON_VELOCITY * sin(item->Pose.Orientation.x) / 2;
			item->Animation.Velocity.z = HARPOON_VELOCITY * cos(item->Pose.Orientation.x) / 2;
			aboveWater = false;
		}
...
void FireGrenade(ItemInfo* laraItem)
{
	...
	if (itemNumber != NO_ITEM)
	{
		...
		item->Pose.Orientation = laraItem->Pose.Orientation + lara->LeftArm.Orientation;

		if (!lara->LeftArm.Locked)
			item->Pose.Orientation += lara->ExtraTorsoRot;

		item->Animation.Velocity.z = GRENADE_VELOCITY;
		item->Animation.Velocity.y = -CLICK(2) * sin(item->Pose.Orientation.x);
...
void GrenadeControl(short itemNumber)
{
		...
		if (item->ItemFlags[1])
		{
			...
			else
			{
				...
				if (newGrenadeItemNumber != NO_ITEM)
				{
					...
					newGrenade->Animation.Velocity.z = 64;
					newGrenade->Animation.Velocity.y = -64 * sin(newGrenade->Pose.Orientation.x);
		...
		TestEnvironment(ENV_FLAG_SWAMP, item->RoomNumber))
	{
		...
		{
			item->Pose.Orientation.z += (((item->Animation.Velocity.z / 16) + 3) * Angle::DegToRad(1.0f));
			if (item->Animation.RequiredState)
				item->Pose.Orientation.y += (((item->Animation.Velocity.z / 4) + 3) * Angle::DegToRad(1.0f));
			else
				item->Pose.Orientation.x += (((item->Animation.Velocity.z / 4) + 3) * Angle::DegToRad(1.0f));
		}
	}
	else
	{
	...
	auto velocity = Vector3Int(
		item->Animation.Velocity.z * sin(item->Animation.TargetState),
		item->Animation.Velocity.y,
		item->Animation.Velocity.z * cos(item->Animation.TargetState)
	);
	...
	else
	{
		...
		float sYrot = item->Pose.Orientation.y;
...
void RocketControl(short itemNumber)
{
	...
	if (TestEnvironment(ENV_FLAG_WATER, item->RoomNumber))
	{
		...
		item->Pose.Orientation.z += ((item->Animation.Velocity.z / 8) + 3) * Angle::DegToRad(1.0f);
		abovewater = false;
	}
	else
	{
...
void FireCrossbow(ItemInfo* laraItem, PHD_3DPOS* pos)
{
	...
	if (itemNumber != NO_ITEM)
	{
		...
		item->Animation.Velocity.z = CLICK(2);
...
void FireCrossBowFromLaserSight(ItemInfo* laraItem, GameVector* src, GameVector* target)
{
	...
	auto angles = GetOrientTowardPoint(Vector3(src->x, src->y, src->z), Vector3(target->x, target->y, target->z));

	auto boltPose = PHD_3DPOS(src->x, src->y, src->z, angles.x, angles.y, angles.z);
