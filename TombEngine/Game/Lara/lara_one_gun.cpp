...
void HarpoonBoltControl(short itemNumber)
{
	...
	if (item->HitPoints == HARPOON_TIME)
	{
		...
		else
		{
			...
			item->Animation.VerticalVelocity = -HARPOON_VELOCITY * sin(item->Pose.Orientation.x) / 2.0f;
			item->Animation.Velocity = HARPOON_VELOCITY * cos(item->Pose.Orientation.x) / 2.0f;
			aboveWater = false;
