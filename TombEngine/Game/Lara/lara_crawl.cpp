...
void lara_as_crouch_idle(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (TrInput & IN_LEFT)
		lara->Control.TurnRate.SetY(-LARA_CRAWL_TURN_MAX);
...
void lara_as_crouch_roll(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (TrInput & IN_LEFT)
	{
		...
		if (lara->Control.TurnRate.GetY() < -LARA_CROUCH_ROLL_TURN_MAX)
			lara->Control.TurnRate.SetY(-LARA_CROUCH_ROLL_TURN_MAX);
...
void lara_as_crawl_forward(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (TrInput & IN_LEFT)
	{
		lara->Control.TurnRate.SetY(lara->Control.TurnRate.GetY() - LARA_CRAWL_MOVE_TURN_RATE);
		if (lara->Control.TurnRate.GetY() < -LARA_CRAWL_MOVE_TURN_MAX)
			lara->Control.TurnRate.SetY(-LARA_CRAWL_MOVE_TURN_MAX);
...
void lara_col_crawl_to_hang(ItemInfo* item, CollisionInfo* coll)
{
	...
	if (item->Animation.AnimNumber == LA_CRAWL_TO_HANG_END)
	{
		...
		TranslateItem(item, item->Pose.Orientation.GetY(), -CLICK(1));
		GetCollisionInfo(coll, item);
