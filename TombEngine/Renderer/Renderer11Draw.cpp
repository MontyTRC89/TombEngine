...
namespace TEN::Renderer
{
	...
	void Renderer11::DrawGunShells(RenderView& view)
	{
		...
		for (int i = 0; i < MAX_GUNSHELL; i++)
		{
			...
			Matrix rotation = Matrix::CreateFromYawPitchRoll(gunshell->pos.Orientation.y, gunshell->pos.Orientation.x, gunshell->pos.Orientation.z);
		    ...
		    for (int i = 0; i < NUM_SPIDERS; i++)
		    {
				...
				if (spider->on)
				{
		            ...
		            for (int iPass = 0; iPass < cPasses; iPass++)
		            {
		                ...
		                drawPrimitives(D3DPT_TRIAngle::DegToRadLIST, 0, 0, bucket->NumVertices, 0, bucket->Indices.size() / 3);
			...
			for (int b = 0; b < mesh->Buckets.size(); b++)
			{
				...
				for (int i = 0; i < NUM_BATS; i++)
				{
					...
					if (bat->On)
					{
						...
						Matrix rotation = Matrix::CreateFromYawPitchRoll(bat->Pose.Orientation.y, bat->Pose.Orientation.x, bat->Pose.Orientation.z);
						Matrix world = rotation * translation;
		...
		if (Objects[ID_LITTLE_BEETLE].loaded)
		{
			...
			for (int i = 0; i < TEN::Entities::TR4::NUM_BEETLES; i++)
			{
				...
				if (beetle->On)
				{
					...
					Matrix rotation = Matrix::CreateFromYawPitchRoll(beetle->Pose.Orientation.y, beetle->Pose.Orientation.x, beetle->Pose.Orientation.z);
					Matrix world = rotation * translation;
		...
		if (Objects[ID_LOCUSTS].loaded)
		{
			...
			for (int i = 0; i < TEN::Entities::TR4::MAX_LOCUSTS; i++)
			{
				...
				if (locust->on)
				{
					...
					Matrix rotation = Matrix::CreateFromYawPitchRoll(locust->pos.Orientation.y, locust->pos.Orientation.x, locust->pos.Orientation.z);
					Matrix world = rotation * translation;
