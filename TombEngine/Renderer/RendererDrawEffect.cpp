...
namespace TEN::Renderer 
{
				...
				abs(ElectricityKnots[0].z) <= ELECTRICITY_RANGE_MAX)
			{
				...
				for (int s = 0; s < laser.NumSegments ; s++)
				{
					...
					AddSpriteBillboardConstrained(
						&_spriteSequenceAssets[ID_DEFAULT_SPRITES].Sprites[SPR_LIGHTHING],
						center,
						color,
	...
	void Renderer::PrepareSmokes(RenderView& view) 
	{
		for (int i = 0; i < 32; i++) 
		{
			...
			if (spark->on) 
			{
				AddSpriteBillboard(&_spriteSequenceAssets[spark->def].Sprites.front(),
		...
		for (int k = 0; k < MAX_FIRE_LIST; k++) 
		{
			...
			if (fire->on) 
			{
				...
				for (int i = 0; i < MAX_SPARKS_FIRE; i++) 
				{
					...
					{
						AddSpriteBillboard(
							&_spriteSequenceAssets[spark->def].Sprites.front(),
	...
	void Renderer::PrepareDrips(RenderView& view)
	{
		...
		for (const auto& drip : Drips)
		{
			...
			AddSpriteBillboardConstrained(
				&_spriteSequenceAssets[ID_DRIP_SPRITE].Sprites.front(),
				drip.Position,
		...
		for (int i = 0; i < MAX_SHOCKWAVE; i++)
		{
			...
			for (int j = 0; j < 16; j++)
			{
				...
				if (shockwave->style == (int)ShockwaveStyle::Normal)
				{
					...
					AddQuad(&_spriteSequenceAssets[ID_DEFAULT_SPRITES].Sprites[SPR_SPLASH],
						pos + p1,
						pos + p2,
						pos + p3,
						pos + p4,
				...
				else if (shockwave->style == (int)ShockwaveStyle::Sophia)
				{
					...
					AddQuad(&_spriteSequenceAssets[ID_DEFAULT_SPRITES].Sprites[SPR_SPLASH3],
						pos + p1,
						pos + p2,
						pos + p3,
						pos + p4,
	...
	void Renderer::PrepareWeatherParticles(RenderView& view) 
	{
		...
		for (auto& p : Weather.GetParticles())
		{
			...
			case WeatherType::None:
				...
				AddSpriteBillboard(
					&_spriteSequenceAssets[ID_DEFAULT_SPRITES].Sprites[SPR_UNDERWATERDUST],
					p.Position,
