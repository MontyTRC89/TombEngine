...
namespace TEN::Renderer
{
	...
	void Renderer::RenderOptionsMenu(Menu menu, int initialY)
	{
		...
		case Menu::OtherSettings:
			...
			AddString(MenuRightSideEntry, y, Str_Enabled(g_Gui.GetCurrentSettings().Configuration.EnableSubtitles), PRINTSTRING_COLOR_WHITE, SF(titleOption == 9));
			GetNextBlockPosition(&y);
			...
			// Mouse smoothing
			AddString(MenuLeftSideEntry, y, g_GameFlow->GetString(STRING_MOUSE_SMOOTHING), PRINTSTRING_COLOR_ORANGE, SF(titleOption == 11));
			AddString(MenuRightSideEntry, y, std::to_string(g_Gui.GetCurrentSettings().Configuration.MouseSmoothing).c_str(), PRINTSTRING_COLOR_WHITE, SF(titleOption == 11));
			GetNextBlockPosition(&y);
