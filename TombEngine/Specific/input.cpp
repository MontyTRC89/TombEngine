...
int S_UpdateInput()
{
	...
	if (Key(KEY_FLARE) || flare)
	{
		if (!flareNo)
		{
			...
			{
				SoundEffect(SFX_TEN_LARA_NO_ENGLISH, nullptr, 2);
				flareNo = true;
			}
			else
			{
		...
		if (medipackTimeout == 0)
		{
			if (LaraItem->HitPoints > 0 && LaraItem->HitPoints < LARA_HEALTH_MAX || Lara.PoisonPotency)
			{
				if (Lara.Inventory.TotalSmallMedipacks != 0)
				{
					...
					SoundEffect(SFX_TEN_MENU_MEDI, nullptr, 2); // TODO: Fix heal sound not triggering if small medi doesn't top off Lara's health. original tr4/5 issue

					if (LaraItem->HitPoints > LARA_HEALTH_MAX)
					{
						LaraItem->HitPoints = LARA_HEALTH_MAX;
						SoundEffect(SFX_TEN_MENU_MEDI, nullptr, 2);
