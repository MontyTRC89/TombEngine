...
unsigned int _stdcall LoadLevel(size_t levelIndex)
{
...
int LoadLevelFile(int levelIndex)
{
	...
	// TODO: remove this variable and use the future to check if the level is loading.
	//
	IsLevelLoading = true;

	// use std::async to use a thread from the system operating thread pool (usually the case)
	// to load the level.

	auto levelLoadingFuture = std::async(std::launch::async, LoadLevel, levelIndex);

	// wait until level has been loaded

	levelLoadingFuture.wait();
