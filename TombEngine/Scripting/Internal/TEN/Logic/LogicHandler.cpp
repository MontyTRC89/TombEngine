...
void LogicHandler::ExecuteLevelScriptFile(const std::string & luaFilename)
{
	m_handler.ExecuteScript(luaFilename);

	// Also load autogenerated node script, if it exists.
	auto nodeScriptName = luaFilename.substr(0, luaFilename.find_last_of(".")) + "_Nodes.lua";
	if (std::filesystem::exists(nodeScriptName))
	{
		m_handler.ExecuteScript("Scripts/NodeFunctions.lua");
		m_handler.ExecuteScript(nodeScriptName);
	}
}
