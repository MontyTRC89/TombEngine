...
namespace TEN::Entities::Vehicles
{
	...
	bool UpvControl(ItemInfo& laraItem, CollisionInfo& coll)
	{
		...
		auto& upvItem = g_Level.Items[player.Vehicle];
		auto& upv = GetUpvInfo(upvItem);
		...
		if (upv.Flags & UPV_FLAG_CONTROL && !(upv.Flags & UPV_FLAG_DEAD))
		{
			...
			else
			{
				...
				{
					player.Air += 10;
					if (player.Air > LARA_AIR_MAX)
						player.Air = LARA_AIR_MAX;
				}
			}
		}
	...
	void UpvUserControl(ItemInfo& upvItem, ItemInfo& laraItem)
	{
		...
		case UPV_STATE_DISMOUNT_WATER_SURFACE:
			if (anim == UPV_ANIM_DISMOUNT_WATER_SURFACE_END && frame == UPV_DISMOUNT_WATER_SURFACE_FRAME)
			{
				...
				player.WaterSurfaceDist = -heightFromWater;
				SetLaraVehicle(laraItem, nullptr);
	...
	bool TestUpvDismount(ItemInfo& upvItem, ItemInfo& laraItem)
	{
		...
		if (player.WaterCurrentPull.x || player.WaterCurrentPull.z)
			return false;
	...
	void UpvBackgroundCollision(ItemInfo& upvItem, ItemInfo& laraItem)
	{
		...
		{
			player.Control.MoveAngle = upvItem.Pose.Orientation.y;
			coll->Setup.ForwardAngle = player.Control.MoveAngle;
		}
		else
		{
			player.Control.MoveAngle = upvItem.Pose.Orientation.y - ANGLE(180.0f);
			coll->Setup.ForwardAngle = player.Control.MoveAngle;
		}
	...
	void DoUpvCurrent(ItemInfo& upvItem, ItemInfo& laraItem)
	{
		...
		{
			int absVel = abs(player.WaterCurrentPull.x);
			int shift;
			if (absVel > 16)
				shift = 4;
			else if (absVel > 8)
			...
			absVel = abs(player.WaterCurrentPull.z);
			if (absVel > 16)
				shift = 4;
			else if (absVel > 8)
				shift = 3;
		...
		upvItem.Pose.Position.x += player.WaterCurrentPull.x / CLICK(1);
