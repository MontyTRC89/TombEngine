...
static void FireUPVHarpoon(ItemInfo* laraItem, ItemInfo* UPVItem)
{
	...
	if (itemNumber != NO_ITEM)
	{
		...
		harpoonItem->Pose.Orientation.Set(UPVItem->Pose.Orientation.GetX(), UPVItem->Pose.Orientation.GetY(), 0.0f);
...
static void UPVControl(ItemInfo* laraItem, ItemInfo* UPVItem)
{
	...
	case UPV_STATE_IDLE:
		...
		if (TrInput & UPV_IN_DISMOUNT && TestUPVDismount(laraItem, UPVItem))
		{
			...
			else
			{
				...
				SoundEffect(SFX_TR3_UPV_STOP, (PoseData*)&UPVItem->Pose.Position.x, SoundEnvironment::Always);
			}
		}
	...
	case UPV_STATE_MOUNT:
		if (anim == UPV_ANIM_MOUNT_SURFACE_END)
		{
			...
			if (frame == MOUNT_SURFACE_SOUND_FRAME)
				SoundEffect(SFX_TR3_UPV_LOOP, (PoseData*)&UPVItem->Pose.Position.x, SoundEnvironment::Always);
	...
	case UPV_STATE_DISMOUNT_WATER_SURFACE:
		...
		{
			UPV->XRot -= UPDOWN_ACCEL;
			if (UPVItem->Pose.Orientation.GetX() < 0)
				UPVItem->Pose.Orientation.SetX();
		}
...
bool UPVControl(ItemInfo* laraItem, CollisionInfo* coll)
{
		...
		lara->Vehicle != NO_ITEM)
	{
		...
		if (UPV->Flags & UPV_CONTROL)
			SoundEffect(SFX_TR3_UPV_LOOP, (PoseData*)&UPVItem->Pose.Position.x, SoundEnvironment::Always, 1.0f + (float)UPVItem->Animation.Velocity / 96.0f);
