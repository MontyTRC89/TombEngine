...
void RubberBoatControl(short itemNumber)
{
	...
	if (rBoatItem->Animation.Velocity > 8)
		SoundEffect(SFX_TEN_VEHICLE_RUBBERBOAT_MOVING, &rBoatItem->Pose, 0, 0.5f + (float)abs(rBoat->Pitch) / (float)RBOAT_MAX_VELOCITY);
	else if (drive)
		SoundEffect(SFX_TEN_VEHICLE_RUBBERBOAT_IDLE, &rBoatItem->Pose, 0, 0.5f + (float)abs(rBoat->Pitch) / (float)RBOAT_MAX_VELOCITY);
