...
namespace TEN::Entities::Vehicles
{
	...
	#define QBIKE_TURN_RATE_ACCEL		  Angle::DegToRad(2.5f)
	#define QBIKE_TURN_RATE_DECEL		  Angle::DegToRad(2.0f)
	#define QBIKE_TURN_RATE_MAX			  Angle::DegToRad(5.0f)
	#define QBIKE_DRIFT_TURN_RATE_ACCEL	  Angle::DegToRad(2.75f)
	#define QBIKE_DRIFT_TURN_RATE_MAX	  Angle::DegToRad(8.0f)
	#define QBIKE_MOMENTUM_TURN_RATE_MIN  Angle::DegToRad(3.0f)
	#define QBIKE_MOMENTUM_TURN_RATE_MAX  Angle::DegToRad(1.5f)
	#define QBIKE_MOMENTUM_TURN_RATE_MAX2 Angle::DegToRad(150.0f) // TODO: Resolve this naming clash!
	...
	static int CanQuadbikeGetOff(ItemInfo* laraItem, int direction)
	{
		...
		angle += (direction < 0) ? -Angle::DegToRad(90.0f) : Angle::DegToRad(90.0f);

		int x = quadBikeItem->Pose.Position.x + CLICK(2) * sin(angle);
		int y = quadBikeItem->Pose.Position.y;
		int z = quadBikeItem->Pose.Position.z + CLICK(2) * cos(angle);
	...
	static int GetQuadCollisionAnim(ItemInfo* quadBikeItem, Vector3Int* pos)
	{
		...
		if (pos->x || pos->z)
		{
			float c = cos(quadBikeItem->Pose.Orientation.y);
			float s = sin(quadBikeItem->Pose.Orientation.y);
	...
	static int QuadDynamics(ItemInfo* quadBikeItem, ItemInfo* laraItem)
	{
		...
		if (quadBikeItem->Pose.Position.y >= probe.Position.Floor)
			speed = quadBikeItem->Animation.Velocity.z * cos(quadBikeItem->Pose.Orientation.x);
		else
			speed = quadBikeItem->Animation.Velocity.z;

		TranslateItem(quadBikeItem, quadBike->MomentumAngle, speed);

		int slip = QBIKE_SLIP * sin(quadBikeItem->Pose.Orientation.x);
		if (abs(slip) > QBIKE_SLIP / 2)
		{
			if (slip > 0)
				...
				slip += 10;
			quadBikeItem->Pose.Position.z -= slip * cos(quadBikeItem->Pose.Orientation.y);
			quadBikeItem->Pose.Position.x -= slip * sin(quadBikeItem->Pose.Orientation.y);
		}

		slip = QBIKE_SLIP_SIDE * sin(quadBikeItem->Pose.Orientation.z);
		if (abs(slip) > QBIKE_SLIP_SIDE / 2)
		{
			quadBikeItem->Pose.Position.z -= slip * sin(quadBikeItem->Pose.Orientation.y);
			quadBikeItem->Pose.Position.x += slip * cos(quadBikeItem->Pose.Orientation.y);
		}
		...
		if (collide)
		{
			newVelocity = (quadBikeItem->Pose.Position.z - old.z) * cos(quadBike->MomentumAngle) + (quadBikeItem->Pose.Position.x - old.x) * sin(quadBike->MomentumAngle);
	...
	bool QuadBikeControl(ItemInfo* laraItem, CollisionInfo* coll)
	{
		...
		short xRot = atan2(QBIKE_FRONT, quadBikeItem->Pose.Position.y - probe.Position.Floor);
		short zRot = atan2(QBIKE_SIDE, probe.Position.Floor - frontLeft.y);
