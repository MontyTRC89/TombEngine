...
static void QuadbikeExplode(ItemInfo* laraItem, ItemInfo* quadItem)
{
	...
	auto pos = PHD_3DPOS(quadItem->Pose.Position.x, quadItem->Pose.Position.y - CLICK(0.5f), quadItem->Pose.Position.z, 0, quadItem->Pose.Orientation.GetY(), 0);
...
static bool QuadCheckGetOff(ItemInfo* laraItem, ItemInfo* quadItem)
{
		...
		TestLastFrame(laraItem))
	{
		...
		TranslateItem(laraItem, laraItem->Pose.Orientation.GetY(), -DISMOUNT_DISTANCE);
...
static int QuadDynamics(ItemInfo* laraItem, ItemInfo* quadItem)
{
	...
	if (collide)
	{
		newVelocity = (quadItem->Pose.Position.z - oldPos.z) * cos(quad->MomentumAngle) + (quadItem->Pose.Position.x - oldPos.x) * sin(quad->MomentumAngle);
		newVelocity *= 256;
