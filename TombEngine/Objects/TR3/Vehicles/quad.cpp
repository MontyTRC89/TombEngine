...
namespace TEN::Entities::Vehicles
{
	...
	static int GetOnQuadBike(ItemInfo* laraItem, ItemInfo* quadItem, CollisionInfo* coll)
	{
		...
		else
		{
			...
			{
				float tempAngle = laraItem->Pose.Orientation.y - quadItem->Pose.Orientation.y;
	...
	bool QuadBikeControl(ItemInfo* laraItem, CollisionInfo* coll)
	{
		...
		quad->Velocity = 0;// DoVehicleWaterMovement(quadItem, laraItem, quad->Velocity, QUAD_RADIUS, &quad->TurnRate);
