...
namespace TEN::Entities::Vehicles
{
	...
	#define MINECART_TERMINAL_ANGLE Angle::DegToRad(22.0f)
	...
	{
		float sinX = sin(minecartItem->Pose.Orientation.x);
		float sinY = sin(minecartItem->Pose.Orientation.y);
		float cosY = cos(minecartItem->Pose.Orientation.y);
		float sinZ = sin(minecartItem->Pose.Orientation.z);
	...
	static void MoveCart(ItemInfo* laraItem, ItemInfo* minecartItem)
	{
		...
		{
			short angle;
			unsigned short rotation = 0;// (((unsigned short)minecartItem->Pose.Orientation.y) / Angle::DegToRad(90.0f)) | (flags.MinecartLeft() * 4);
			...
			angle = Angle::RadToShrt(Angle::OrientBetweenPoints(minecartItem->Pose.Position.ToVector3(), Vector3(minecart->TurnX, 0.0f, minecart->TurnZ))) & 0x3fff;
		...
		if (minecart->Flags & (MINECART_FLAG_TURNING_LEFT | MINECART_FLAG_TURNING_RIGHT))
		{
			...
			if (minecart->Flags & (MINECART_FLAG_TURNING_LEFT | MINECART_FLAG_TURNING_RIGHT))
			{
				unsigned short quadrant = ((unsigned short)minecartItem->Pose.Orientation.y) / Angle::DegToRad(90.0f); // TODO: Use GetQuadrant()?
				unsigned short degree = 0;// minecartItem->Pose.Orientation.y & 16383;
		...
		if (minecart->Flags & (MINECART_FLAG_TURNING_LEFT | MINECART_FLAG_TURNING_RIGHT))
		{
			...
			if (minecart->Flags & MINECART_FLAG_TURNING_RIGHT)
				...
				minecartItem->Pose.Orientation.z = ((Angle::DegToRad(90.0f) - val) * minecartItem->Animation.Velocity) / 512;
		}
		else
	...
	static void DoUserInput(ItemInfo* minecartItem, ItemInfo* laraItem, MinecartInfo* minecart)
	{
		...
		case MINECART_STATE_DISMOUNT_LEFT:
				...
				laraItem->Animation.FrameNumber == g_Level.Anims[laraItem->Animation.AnimNumber].frameEnd)
			{
				...
				laraItem->Pose.Orientation = EulerAngles(0.0f, minecartItem->Pose.Orientation.y + Angle::DegToRad(90.0f), 0.0f);
	...
	void MinecartCollision(short itemNumber, ItemInfo* laraItem, CollisionInfo* coll)
	{
		...
		if (doMount)
		{
			...
			float angle = Angle::Normalize((Angle::OrientBetweenPoints(minecartItem->Pose.Position.ToVector3(), laraItem->Pose.Position.ToVector3()) - minecartItem->Pose.Orientation.GetY()));
