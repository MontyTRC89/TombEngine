						...
						if (TestBoundsCollide(item, laraItem, CART_ENTITY_RADIUS))
						{
							if (item->ObjectNumber == ID_ANIMATING2)
							{
										...
										laraItem->Animation.AnimNumber == Objects[ID_MINECART_LARA_ANIMS].animIndex + 6))
								{
									...
									{
										SoundEffect(SFX_TR5_SWORD_GOD_HIT_METAL, &item->Pose, 2);
...
static void MoveCart(ItemInfo* laraItem, ItemInfo* minecartItem)
{
	...
	if (minecartItem->Animation.Velocity < CART_MIN_VEL)
	{
		...
		StopSoundEffect(SFX_TEN_VEHICLE_MINECART_TRACK_LOOP);

		if (minecart->VerticalVelocity)
			StopSoundEffect(SFX_TEN_VEHICLE_MINECART_PULLY_LOOP);
		else
			SoundEffect(SFX_TEN_VEHICLE_MINECART_PULLY_LOOP, &minecartItem->Pose, 2);
	}
	else
	{
		StopSoundEffect(SFX_TEN_VEHICLE_MINECART_PULLY_LOOP);
		...
		if (minecart->VerticalVelocity)
			...
			SoundEffect(SFX_TEN_VEHICLE_MINECART_TRACK_LOOP, &minecartItem->Pose, (2 | 4) + 0x1000000 + (minecartItem->Animation.Velocity * 32768));
	}
	...
	else
	{
		if (minecartItem->Pose.Position.y > minecart->FloorHeightMiddle)
		{
			if (minecart->VerticalVelocity > 0)
				SoundEffect(SFX_TEN_VEHICLE_QUADBIKE_FRONT_IMPACT, &minecartItem->Pose, 2);
...
static void DoUserInput(ItemInfo* minecartItem, ItemInfo* laraItem, MinecartInfo* minecart)
{
	...
	case CART_STATE_IDLE:
		...
		{
			SoundEffect(SFX_TEN_VEHICLE_MINECART_START, &minecartItem->Pose, 2);
	...
	case CART_STATE_BRAKING:
		...
		{
			laraItem->Animation.TargetState = CART_STATE_DUCK;
			StopSoundEffect(SFX_TEN_VEHICLE_MINECART_BRAKE);
		}
		...
		else
		{
			...
			SoundEffect(SFX_TEN_VEHICLE_MINECART_BRAKE, &laraItem->Pose, 2);
		}
