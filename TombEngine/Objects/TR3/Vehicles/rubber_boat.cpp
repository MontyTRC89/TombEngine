...
namespace TEN::Entities::Vehicles
{
	...
	#define RBOAT_TURN_RATE_ACCEL (Angle::DegToRad(0.25f) / 2)
	#define RBOAT_TURN_RATE_DECEL Angle::DegToRad(0.25f)
	#define RBOAT_TURN_RATE_MAX	  Angle::DegToRad(4.0f)
	...
	static int GetRubberBoatCollisionAnim(ItemInfo* rBoatItem, Vector3Int* moved)
	{
		...
		if (moved->x || moved->z)
		{
			float sinY = sin(rBoatItem->Pose.Orientation.y);
			float cosY = cos(rBoatItem->Pose.Orientation.y);
	...
	static int RubberBoatDynamics(short itemNumber, ItemInfo* laraItem)
	{
		...
		rBoatItem->Pose.Position.z += rBoatItem->Animation.Velocity.z * cos(rBoatItem->Pose.Orientation.y);
		rBoatItem->Pose.Position.x += rBoatItem->Animation.Velocity.z * sin(rBoatItem->Pose.Orientation.y);
		if (rBoatItem->Animation.Velocity.z >= 0)
			rBoat->PropellerRotation += (rBoatItem->Animation.Velocity.z * Angle::DegToRad(3.0f)) + Angle::DegToRad(2.0f);
		else
		...
		if (slip || collide)
		{
			int newVelocity = (rBoatItem->Pose.Position.z - old.z) * cos(rBoatItem->Pose.Orientation.y) + (rBoatItem->Pose.Position.x - old.x) * sin(rBoatItem->Pose.Orientation.y);
	...
	void RubberBoatControl(short itemNumber)
	{
		...
		if (rBoatItem->Animation.Velocity.z &&
			...
			height != NO_HEIGHT)
		{
			TriggerRubberBoatMist(prop.x, prop.y, prop.z, abs(rBoatItem->Animation.Velocity.z), rBoatItem->Pose.Orientation.y + Angle::DegToRad(180.0f), 0);
