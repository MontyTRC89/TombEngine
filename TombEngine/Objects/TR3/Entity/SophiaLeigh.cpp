...
namespace TEN::Entities::Creatures::TR3
{
	...
	{
		if ((item.Animation.AnimNumber == SOPHIALEIGH_ANIM_SUMMON_START && item.Animation.FrameNumber > GetFrameIndex(&item, 6)) ||
			 item.Animation.AnimNumber == SOPHIALEIGH_ANIM_SUMMON ||
			(item.Animation.AnimNumber == SOPHIALEIGH_ANIM_SUMMON_END && item.Animation.FrameNumber < GetFrameIndex(&item, 3)) ||
			(item.Animation.AnimNumber == SOPHIALEIGH_ANIM_SCEPTER_SHOOT && item.Animation.FrameNumber > GetFrameIndex(&item, 39) && item.Animation.FrameNumber < GetFrameIndex(&item, 47)) ||
			(item.Animation.AnimNumber == SOPHIALEIGH_ANIM_SCEPTER_SMALL_SHOOT && item.Animation.FrameNumber > GetFrameIndex(&item, 14) && item.Animation.FrameNumber < GetFrameIndex(&item, 18)))
		{
			TriggerDynamicLight(
				pos.x, pos.y, pos.z,
		...
		else if (item.Animation.AnimNumber == SOPHIALEIGH_ANIM_SUMMON_END && item.Animation.FrameNumber >= GetFrameIndex(&item, 3) && item.ItemFlags[1] > 0)
		{
			TriggerDynamicLight(
				pos.x, pos.y, pos.z,
				item.ItemFlags[1] + SOPHIALEIGH_LIGHTNING_GLOW_SIZE,
