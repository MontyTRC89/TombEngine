...
namespace TEN::Entities::Creatures::TR3
{
	...
	static void TriggerLaserBolt(ItemInfo* item, ItemInfo* enemy, const BiteInfo& bite, bool isBigLaser, short angleAdd)
	{
		...
		if (enemy->IsLara())
		{
			...
			if (lara->Control.IsLow)
				fx.pos.Orientation.x = item->Pose.Orientation.x - STEPUP_HEIGHT;
			else
	...
	static void LondonBossTowerControl(ItemInfo* item, CreatureInfo* creature, SophiaData* data)
	{
		...
		case SOPHIALEIGH_STATE_BIG_SHOOT:
			...
			{
				TriggerLaserBolt(item, creature->Enemy, SOPHIALEIGH_Right, false, ANGLE(1.5f));
				TriggerLaserBolt(item, creature->Enemy, SOPHIALEIGH_Staff, true, 0);
				TriggerLaserBolt(item, creature->Enemy, SOPHIALEIGH_Left, false, -ANGLE(1.5f));
			}
	...
	void LondonBossControl(short itemNumber)
	{
		...
		if (item->TestFlagField((int)BossItemFlags::ImmortalState, TRUE) && item->TestOcb((int)SophiaMode::Tower))
