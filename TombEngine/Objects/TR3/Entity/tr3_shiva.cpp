...
enum ShivaAnim
{
	...
	SHIVA_ANIM_ATTACK_GRAB = 17,
	SHIVA_ANIM_KILL = 18,
	SHIVA_ANIM_ATTACK = 19,
...
void ShivaControl(short itemNumber)
{
	...
	else
	{
		...
		case SHIVA_STATE_ATTACK_1:
			...
			{
				extraHeadRot.SetY(AI.angle);
				extraTorsoRot.Set(AI.xAngle, AI.angle, 0.0f);
			}
		...
		case SHIVA_STATE_ATTACK_2:
			shiva->MaxTurn = Angle::DegToRad(4.0f);
			extraHeadRot.SetY(AI.angle);
			extraTorsoRot.SetY(AI.angle);

			if (AI.xAngle > 0)
				extraTorsoRot.SetX(AI.xAngle);
	...
	extraHeadRot.SetY(extraHeadRot.GetY() - extraTorsoRot.GetY());

	CreatureJoint(item, 0, extraTorsoRot.GetY());
	CreatureJoint(item, 1, extraTorsoRot.GetX());
	CreatureJoint(item, 2, extraHeadRot.GetY());
	CreatureJoint(item, 3, extraHeadRot.GetX());
