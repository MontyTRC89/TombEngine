...
namespace TEN::Entities::Creatures::TR3
{
	...
	constexpr auto GUN_TURRET_HIT_POINTS_MAX = 28;
	...
	void ControlGunTurret(short itemNumber)
	{
		...
		if (turret.MuzzleFlash[0].Delay >= 1)
			turret.MuzzleFlash[0].Delay--;

		if (turret.MuzzleFlash[1].Delay >= 1)
			turret.MuzzleFlash[1].Delay--;
		...
		case AUTO_GUN_STATE_FIRE:
			...
			else if (item.Animation.FrameNumber == GetAnimData(item).frameBase)
			{
				...
				if (turret.MuzzleFlash[0].Delay == 0)
				{
					...
					turret.MuzzleFlash[0].ApplyXRotation = false;
					turret.MuzzleFlash[0].ApplyZRotation = true;
					turret.MuzzleFlash[0].UseSmoke = true;
				...
				if (turret.MuzzleFlash[1].Delay == 0)
				{
					...
					turret.MuzzleFlash[1].ApplyXRotation = false;
					turret.MuzzleFlash[1].ApplyZRotation = true;
					turret.MuzzleFlash[1].UseSmoke = true;					
					turret.MuzzleFlash[1].Delay = 1;
				}
		...
		turret.JointRotation[0] += deltaAngle;
