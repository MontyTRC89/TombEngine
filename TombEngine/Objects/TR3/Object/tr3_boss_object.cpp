...
namespace TEN::Entities::Object::TR3
{
	void BOSS_SpawnShield(const ItemInfo& item, const Vector4& color)
	{
		...
		shieldItem.Model.Mutator[0].Scale = Vector3(2.0f); // Change model size target be * 2.
	...
	void BOSS_TriggerExplosionSmoke(const Vector3& pos)
	{
	...
	void BOSS_CheckForRequiredObjects(ItemInfo& item)
	{
		...
		// These are only for rendering effects. Not required but highly recommended for nice look in-game.
