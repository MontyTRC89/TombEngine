...
namespace TEN::Entities::TR1
{
	...
	#define WINGED_MUTANT_WALK_FORWARD_TURN_RATE_MAX Angle::DegToRad(2.0f)
	#define WINGED_MUTANT_RUN_FORWARD_TURN_RATE_MAX	 Angle::DegToRad(6.0f)
	...
	WingedMutantProjectileType CanTargetLara(ItemInfo* item, CreatureInfo* creature, AI_INFO* AI)
	{
		...
		{
			if ((AI->angle > 0 && AI->angle < Angle::DegToRad(45.0f)) &&
				item->TestFlags(WMUTANT_OCB_DISABLE_DART_WEAPON, false))
			{
				return WMUTANT_PROJ_DART;
			}
			else if ((AI->angle < 0 && AI->angle > Angle::DegToRad(-45.0f)) &&
