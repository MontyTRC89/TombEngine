		...
		if (item->ItemFlags[0])
		{
			...
			else
			{
				...
				else
				{
					...
					if (JustLoaded)
					{
						int c = 8192 * cos(item->Pose.Orientation.x + Angle::DegToRad(18.3f));

						dest.x = LaserHeadData.target.x = src.x + c * sin(item->Pose.Orientation.y);
						dest.y = LaserHeadData.target.y = src.y + SECTOR(8) * sin(3328 - item->Pose.Orientation.x);
				...
				auto angles = EulerAngles::Zero;// GetVectorAngles(LaserHeadData.target.x - src.x, LaserHeadData.target.y - src.y, LaserHeadData.target.z - src.z);
