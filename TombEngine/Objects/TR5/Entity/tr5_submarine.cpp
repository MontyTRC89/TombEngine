...
void ChaffFlareControl(short itemNumber)
{
	...
	int dx = item->Animation.Velocity.z * sin(item->Pose.Orientation.y);
	int dz = item->Animation.Velocity.z * cos(item->Pose.Orientation.y);
...
void TorpedoControl(short itemNumber)
{
	...
	item->Pose.Orientation.z += Angle::DegToRad(0.09f) * item->Animation.Velocity.z;

	int c = item->Animation.Velocity.z * cos(item->Pose.Orientation.x);
