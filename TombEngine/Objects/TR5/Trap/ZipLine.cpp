...
namespace TEN::Traps::TR5
{
	...
	const auto ZipLineMountBasis = InteractionBasis
	{
		GameBoundingBox(
	...
	void CollideZipLine(short itemNumber, ItemInfo* laraItem, CollisionInfo* coll)
	{
		...
		{
			if (TestPlayerEntityInteract(&zipLineItem, laraItem, ZipLineMountBasis))
			{
				if (AlignPlayerToEntity(&zipLineItem, laraItem, ZipLineMountedOffset))
				{
