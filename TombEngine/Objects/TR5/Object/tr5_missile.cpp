	...
	if (fx->pos.Position.y >= probe.Position.Floor || fx->pos.Position.y <= probe.Position.Ceiling)
	{
		...
		if (fx->flag1)
		{
			if (fx->flag1 == 1)
			{
				...
				TriggerShockwave(&fx->pos, 48, 256, 64, 64, 128, 0, 24, EulerAngles::Zero, 1, true);
				fx->pos.Position.y -= 128;
				TriggerShockwave(&fx->pos, 48, 256, 48, 64, 128, 0, 24, EulerAngles::Zero, 1, true);
			}
			else if (fx->flag1 == 2)
			{
				ExplodeFX(fx, 0, 32);
		...
		else
		{
			...
			TriggerShockwave(&fx->pos, 48, 240, 48, 0, 96, 128, 24, EulerAngles::Zero, 2, true);
		}
	...
	else if (ItemNearLara(fx->pos.Position, 200))
	{
		if (fx->flag1)
		{
			if (fx->flag1 == 1)
			{
				...
				TriggerExplosionSparks(x, y, z, 3, -2, 2, fx->roomNumber);
				fx->pos.Position.y -= 64;
				TriggerShockwave(&fx->pos, 48, 256, 64, 0, 128, 64, 24, EulerAngles::Zero, 1, true);
				fx->pos.Position.y -= 128;
				TriggerShockwave(&fx->pos, 48, 256, 48, 0, 128, 64, 24, EulerAngles::Zero, 1, true);
				KillEffect(itemNumber);
		...
		else
		{
			// HYDRA hit effect
			TriggerExplosionSparks(x, y, z, 3, -2, 0, fx->roomNumber);
			TriggerShockwave(&fx->pos, 48, 240, 48, 0, 96, 128, 24, EulerAngles::Zero, 0, true);
