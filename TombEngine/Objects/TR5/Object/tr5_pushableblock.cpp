...
namespace TEN::Entities::Generic
{
	...
	static PushableSoundData GetPushableSfxData(MaterialType material)
	{
		static const auto SOUND_DATA_DEFAULT = PushableSoundData{ SFX_TR4_PUSHABLE_SOUND, SFX_TR4_PUSH_BLOCK_END, SFX_TR4_BOULDER_FALL };
	...
	bool PushableBlockManageGravity(int itemNumber)
	{
		...
		case PushableGravityState::Falling:
			...
			else
			{
				...
				SoundEffect(GetPushableSfx(Fall, pos), &pushableItem.Pose, SoundEnvironment::Always);
	...
	void PushableBlockManageMoving(int itemNumber)
	{
		...
		bool isPlayerPulling = (LaraItem->Animation.AnimNumber == LA_PUSHABLE_PULL ||
							  LaraItem->Animation.AnimNumber == LA_PUSHABLE_BLOCK_PULL);
		...
		else
		{
			...
			if (IsHeld(In::Action) &&
				IsNextSectorValid(pushableItem, nextPos, isPlayerPulling))
			{
				pushable.StartPos = pushableItem.Pose.Position;
				pushable.StartPos.RoomNumber = pushableItem.RoomNumber;
	...
	int GetPushableSfx(PushableSoundType soundType, const GameVector& pos)
	{
		...
		default:
			TENLog("Requested missing pushable sfx.", LogLevel::Error, LogConfig::All, true);
			return 0;
		}
	}
	...
	void PushablesManageSounds(int itemNumber)
	{
		...
		if (pushable.CurrentSoundState == PushableSoundState::Moving)
		{
			SoundEffect(GetPushableSfx(Loop, pos), &pushableItem.Pose, SoundEnvironment::Always);
		}
		else if (pushable.CurrentSoundState == PushableSoundState::Stopping)
		{
			pushable.CurrentSoundState = PushableSoundState::None;
			SoundEffect(GetPushableSfx(Stop, pos), &pushableItem.Pose, SoundEnvironment::Always);
		}
	}
	...
	void UpdatePushablesRoomNumbers(int itemNumber)
	{
		...
		// Check and update room number of pushables and others linked in stack.
		auto pointColl = GetCollision(&pushableItem);
	...
	bool IsNextSectorValid(ItemInfo& pushableItem, const GameVector& target, bool checkIfPlayerFits)
	{
	...
	void MoveStack(const int itemNumber, const Vector3i& target)
	{
		...
		while (true)
		{
			...
			pushableLinkedItem.Pose.Position.y += target.y; // Vertical movement receives velocity, not fixed value.
	...
	void ManageStackBridges(int itemNumber, bool addBridge)
	{
		...
		while (pushableLinked.StackUpperItem != NO_ITEM)
		{
			if (pushableLinked.UsesRoomCollision)
				addBridge ? AddBridge(pushableLinkedItem.Index) : RemoveBridge(pushableLinkedItem.Index);
