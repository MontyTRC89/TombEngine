...
namespace TEN::Entities::Generic
{
	...
	void PushableBlockControl(short itemNumber)
	{
		...
		case LA_PUSHABLE_PUSH:
			displaceDepth = GetLastKeyframe(GAME_OBJECT_ID::ID_LARA, LaraItem->Animation.AnimNumber).BoundingBox.Z2;
			...
			if (LaraItem->Animation.FrameNumber == (GetAnimData(*LaraItem).Keyframes.size() - 2))
			{
	...
	bool TestBlockPull(ItemInfo* item, int blockHeight, int quadrant)
	{
		...
		const auto& frameOffset = GetClosestKeyframe(*LaraItem).Offset;
