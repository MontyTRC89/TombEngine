	...
	void SkidooManControl(short riderItemNumber)
	{
		...
		if (skidoo->HitPoints <= 0)
		{
			if (rider->Animation.ActiveState != SMAN_STATE_DEATH)
			{
				...
				rider->Animation.FrameNumber =  GetAnimData(riderItem).frameBase;
		...
		if (rider->Animation.ActiveState != SMAN_STATE_DEATH)
		{
			...
			rider->Animation.FrameNumber = skidoo->Animation.FrameNumber + (GetAnimData(rider).frameBase - GetAnimData(skidoo).frameBase);
		}
