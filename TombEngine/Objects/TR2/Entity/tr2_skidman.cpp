...
void SkidooManControl(short riderItemNumber)
{
	...
	if (riderItem->Animation.ActiveState != SMAN_STATE_DEATH)
	{
		...
		{
			SoundEffect(SFX_TR4_BADDY_TROOP_UZI, &item->Pose, 0);
			creatureInfo->Flags--;
		}
	}
	...
	{
		SoundEffect(SFX_TEN_VEHICLE_SNOWMOBILE_IDLE, &item->Pose, 0);
	...
	else
	{
		...
		DoSnowEffect(item);
		SoundEffect(SFX_TEN_VEHICLE_SNOWMOBILE_IDLE, &item->Pose, 4 + ((0x10000 - (100 - item->Animation.Velocity) * 100) << 8));
	}
