...
namespace TEN::Entities::Creatures::TR2
{
	...
	const auto SILENCER_WALK_TURN_RATE_MAX = ANGLE(5.0f);
	const auto SILENCER_RUN_TURN_RATE_MAX  = ANGLE(5.0f);
		...
		else
		{
			...
			case SILENCER_STATE_IDLE:
				...
				else
				{
					...
					{
						item->Animation.TargetState = SILENCER_STATE_IDLE_FRAME;
						item->Animation.RequiredState = Random::TestProbability(0.5f) ? SILENCER_STATE_AIM_1 : SILENCER_STATE_AIM_2;
					}
			...
			case SILENCER_STATE_RUN_FORWARD:
				...
				if (Targetable(item, &AI))
				{
					...
					item->Animation.TargetState = Random::TestProbability(0.5f) ? SILENCER_STATE_RUN_FORWARD : SILENCER_STATE_IDLE_FRAME;
				else
					item->Animation.TargetState = SILENCER_STATE_IDLE_FRAME;
