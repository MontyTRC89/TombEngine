...
bool TestSkidooDismountOK(ItemInfo* skidooItem, int direction)
{
	...
	int z = skidooItem->Pose.Position.z - SKIDOO_DISMOUNT_DISTANCE * cos(angle);
...
bool TestSkidooDismount(ItemInfo* laraItem, ItemInfo* skidooItem)
{
	...
	if (lara->Vehicle != NO_ITEM)
	{
			...
			laraItem->Animation.FrameNumber == g_Level.Anims[laraItem->Animation.AnimNumber].frameEnd)
		{
			...
			laraItem->Pose.Position.z -= SKIDOO_DISMOUNT_DISTANCE * cos(laraItem->Pose.Orientation.GetY());
			...
			(skidooItem->Pose.Position.y == skidooItem->Floor || TestLastFrame(laraItem)))
		{
			...
			lara->Control.HandStatus = HandStatus::Free;
			lara->Control.MoveAngle = skidooItem->Pose.Orientation.GetY();
...
int GetSkidooCollisionAnim(ItemInfo* skidooItem, Vector3Int* moved)
{
	...
	if (moved->x || moved->z)
	{
		float s = sin(skidooItem->Pose.Orientation.GetY());
		float c = cos(skidooItem->Pose.Orientation.GetY());
...
int SkidooDynamics(ItemInfo* laraItem, ItemInfo* skidooItem)
{
	...
	int slip = SKIDOO_SLIP * sin(skidooItem->Pose.Orientation.GetX());
	if (abs(slip) > (SKIDOO_SLIP / 2))
	{
		skidooItem->Pose.Position.z -= slip * cos(skidooItem->Pose.Orientation.GetY());
	...
	if (abs(slip) > (SKIDOO_SLIP_SIDE / 2))
	{
		...
		skidooItem->Pose.Position.x += slip * cos(skidooItem->Pose.Orientation.GetY());
	}
	...
	if (collide)
	{
		int newVelocity = (skidooItem->Pose.Position.z - oldPos.z) * cos(skidoo->MomentumAngle) + (skidooItem->Pose.Position.x - oldPos.x) * sin(skidoo->MomentumAngle);
