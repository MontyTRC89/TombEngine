	...
	if (lara->Vehicle != NO_ITEM)
	{
			...
			(skidooItem->Pose.Position.y == skidooItem->Floor || TestLastFrame(laraItem)))
		{
			...
			else
			{
				...
				SoundEffect(SFX_TEN_LARA_FALL, &laraItem->Pose, 0);
			}
...
void SkidooExplode(ItemInfo* laraItem, ItemInfo* skidooItem)
{
	...
	SoundEffect(SFX_TEN_EXPLOSION1, 0, 0);
	SoundEffect(SFX_TEN_EXPLOSION2, 0, 0);
...
bool SkidooControl(ItemInfo* laraItem, CollisionInfo* coll)
{
	...
	if (drive > 0)
	{
		...
		SoundEffect(skidoo->Pitch ? SFX_TEN_VEHICLE_SNOWMOBILE_MOVING : SFX_TEN_VEHICLE_SNOWMOBILE_ACCELERATE, &skidooItem->Pose, 0, 0.5f + skidoo->Pitch / (float)SKIDOO_MAX_VELOCITY);
	}
	else
	{
		skidoo->TrackMesh = 0;
		if (!drive)
			SoundEffect(SFX_TEN_VEHICLE_SNOWMOBILE_IDLE, &skidooItem->Pose, 0);
		skidoo->Pitch = 0;
	}
...
void SkidooAnimation(ItemInfo* laraItem, ItemInfo* skidooItem, int collide, bool dead)
{
		...
		collide && !dead)
	{
		if (laraItem->Animation.ActiveState != SKIDOO_STATE_HIT)
		{
			if (collide == SKIDOO_ANIM_HIT_FRONT)
				SoundEffect(SFX_TEN_VEHICLE_IMPACT_1, &skidooItem->Pose, 0);
			else
				SoundEffect(SFX_TEN_VEHICLE_IMPACT_2, &skidooItem->Pose, 0);
	...
	else
	{
		...
		case SKIDOO_STATE_FALL:
			...
			{
				laraItem->Animation.TargetState = SKIDOO_STATE_SIT;
				SoundEffect(SFX_TEN_VEHICLE_IMPACT_3, &skidooItem->Pose, 0);
			}
