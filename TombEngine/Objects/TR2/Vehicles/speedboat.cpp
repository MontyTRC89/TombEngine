...
namespace TEN::Entities::Vehicles
{
	...
	#define SPEEDBOAT_TURN_RATE_ACCEL (Angle::DegToRad(0.25f) / 2)
	#define SPEEDBOAT_TURN_RATE_DECEL Angle::DegToRad(0.25f)
	#define SPEEDBOAT_TURN_RATE_MAX	  Angle::DegToRad(4.0f)
	...
	bool TestSpeedboatDismount(ItemInfo* speedboatItem, int direction)
	{
		short angle;
		if (direction < 0)
			angle = speedboatItem->Pose.Orientation.y - Angle::DegToRad(90.0f);
		else
			angle = speedboatItem->Pose.Orientation.y + Angle::DegToRad(90.0f);

		int x = speedboatItem->Pose.Position.x +  SPEEDBOAT_DISMOUNT_DISTANCE * sin(angle);
		int y = speedboatItem->Pose.Position.y;
		int z = speedboatItem->Pose.Position.z +  SPEEDBOAT_DISMOUNT_DISTANCE * cos(angle);
	...
	int GetSpeedboatHitAnim(ItemInfo* speedboatItem, Vector3Int* moved)
	{
		...
		if (moved->x || moved->z)
		{
			float sinY = sin(speedboatItem->Pose.Orientation.y);
			float cosY = cos(speedboatItem->Pose.Orientation.y);
	...
	int SpeedboatDynamics(short itemNumber, ItemInfo* laraItem)
	{
		...
		speedboatItem->Pose.Position.x += speedboatItem->Animation.Velocity.z * sin(speedboatItem->Pose.Orientation.y);
		speedboatItem->Pose.Position.z += speedboatItem->Animation.Velocity.z * cos(speedboatItem->Pose.Orientation.y);
		...
		if (slip || collide)
		{
			newVelocity = (speedboatItem->Pose.Position.z - old.z) * cos(speedboatItem->Pose.Orientation.y) + (speedboatItem->Pose.Position.x - old.x) * sin(speedboatItem->Pose.Orientation.y);
		...
		else
		{
			...
			case SPEEDBOAT_STATE_IDLE:
				if (TrInput & VEHICLE_IN_DISMOUNT)
				{
					if (speedboatItem->Animation.Velocity.z == 0)
					{
						if (TrInput & VEHICLE_IN_RIGHT && TestSpeedboatDismount(speedboatItem, speedboatItem->Pose.Orientation.y + Angle::DegToRad(90.0f)))
							laraItem->Animation.TargetState = SPEEDBOAT_STATE_DISMOUNT_RIGHT;
						else if (TrInput & VEHICLE_IN_LEFT && TestSpeedboatDismount(speedboatItem, speedboatItem->Pose.Orientation.y - Angle::DegToRad(90.0f)))
	...
	void SpeedboatControl(short itemNumber)
	{
		...
		short xRot = atan2(SPEEDBOAT_FRONT, speedboatItem->Pose.Position.y - probe.Position.Floor);
		short zRot = atan2(SPEEDBOAT_SIDE, probe.Position.Floor - frontLeft.y);
