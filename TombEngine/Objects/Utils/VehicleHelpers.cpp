...
namespace TEN::Entities::Vehicles
{
	VehicleMountType GetVehicleMountType(ItemInfo* vehicleItem, ItemInfo* laraItem, CollisionInfo* coll, vector<VehicleMountType> allowedMountTypes, float maxDistance2D, float maxVerticalDistance)
	{
		...
		for (auto mountType : allowedMountTypes)
		{
			...
			case VehicleMountType::Front:
				if (hasInputAction &&
					deltaHeadingAngle > Angle::DegToRad(135.0f) && deltaHeadingAngle < -Angle::DegToRad(135.0f) &&
					onCorrectSide &&
			...
			case VehicleMountType::Back:
				if (hasInputAction &&
					abs(deltaHeadingAngle) < Angle::DegToRad(45.0f) &&
					onCorrectSide &&
					!laraItem->Animation.IsAirborne)
				{
					break;
			...
			case VehicleMountType::Left:
				if (hasInputAction &&
					deltaHeadingAngle > -Angle::DegToRad(135.0f) && deltaHeadingAngle < -Angle::DegToRad(45.0f) &&
					onCorrectSide &&
					!laraItem->Animation.IsAirborne)
				{
					break;
			...
			case VehicleMountType::Right:
				if (hasInputAction &&
					deltaHeadingAngle > Angle::DegToRad(45.0f) && deltaHeadingAngle < Angle::DegToRad(135.0f) &&
					onCorrectSide &&
					!laraItem->Animation.IsAirborne)
				{
					break;
	...
	{
		float sinX = sin(vehicleItem->Pose.Orientation.x);
		float sinY = sin(vehicleItem->Pose.Orientation.y);
		float cosY = cos(vehicleItem->Pose.Orientation.y);
		float sinZ = sin(vehicleItem->Pose.Orientation.z);
	...
	{
		Matrix world =
			Matrix::CreateFromYawPitchRoll(vehicleItem->Pose.Orientation.y, vehicleItem->Pose.Orientation.x, vehicleItem->Pose.Orientation.z) *
	...
	{
		if (abs(vehicleItem->Pose.Orientation.z) > Angle::DegToRad(0.1f))
