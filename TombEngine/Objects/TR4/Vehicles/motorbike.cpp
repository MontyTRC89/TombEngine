...
namespace TEN::Entities::Vehicles
{
	...
	#define MOTORBIKE_FORWARD_TURN_ANGLE Angle::DegToRad(1.5f)
	#define MOTORBIKE_BACK_TURN_ANGLE Angle::DegToRad(0.5f)
	#define MOTORBIKE_TURN_ANGLE_MAX Angle::DegToRad(5.0f)
	#define MOTORBIKE_MOMENTUM_TURN_ANGLE_MIN Angle::DegToRad(4.0f)
	#define MOTORBIKE_MOMENTUM_TURN_ANGLE_MAX Angle::DegToRad(1.5f)
	#define MOTORBIKE_MOMENTUM_TURN_ANGLE_MAX2 Angle::DegToRad(150.0f)
	...
	#define MOTORBIKE_IN_DISMOUNT	(IN_JUMP | IN_ROLL)
	...
	{
		float sinX = sin(motorbikeItem->Pose.Orientation.x);
		float sinY = sin(motorbikeItem->Pose.Orientation.y);
		float cosY = cos(motorbikeItem->Pose.Orientation.y);
		float sinZ = sin(motorbikeItem->Pose.Orientation.z);
	...
	static bool TestMotorbikeMount(short itemNumber, ItemInfo* laraItem)
	{
		...
		short angle = atan2(motorbikeItem->Pose.Position.z - laraItem->Pose.Position.z, motorbikeItem->Pose.Position.x - laraItem->Pose.Position.x) - motorbikeItem->Pose.Orientation.y;
		...
		{
			if (deltaAngle > -Angle::DegToRad(45.0f) && angle < Angle::DegToRad(135.0f))
				return false;
		}
		// Right.
		else
	...
	static void DrawMotorBikeSmoke(ItemInfo* motorbikeItem, ItemInfo* laraItem)
	{
		...
		if (laraItem->Animation.ActiveState != MOTORBIKE_STATE_MOUNT && laraItem->Animation.ActiveState != MOTORBIKE_STATE_DISMOUNT)
		{
			...
			{
				TriggerMotorbikeExhaustSmoke(pos.x, pos.y, pos.z, motorbikeItem->Pose.Orientation.y - Angle::DegToRad(180.0f), 64 - speed, true);
				return;
			}
			...
			TriggerMotorbikeExhaustSmoke(pos.x, pos.y, pos.z, motorbikeItem->Pose.Orientation.y - Angle::DegToRad(180.0f), speed, false);
		}
	}
	...
	static int MotorBikeDynamics(ItemInfo* motorbikeItem, ItemInfo* laraItem)
	{
		...
		{
			if (motorbike->TurnRate < -Angle::DegToRad(0.5f))
				motorbike->TurnRate += Angle::DegToRad(0.5f);
			else if (motorbike->TurnRate > Angle::DegToRad(0.5f))
				motorbike->TurnRate -= Angle::DegToRad(0.5f);
			else
				motorbike->TurnRate = 0;
		...
		{
			if (motorbike->TurnRate >= -Angle::DegToRad(1.0f))
			{
				if (motorbike->TurnRate <= Angle::DegToRad(1.0f))
					motorbike->TurnRate = 0;
				else
					motorbike->TurnRate -= Angle::DegToRad(1.0f);
			}
			else
				motorbike->TurnRate += Angle::DegToRad(1.0f);
		...
		if (motorbikeItem->Pose.Position.y >= floorHeight)
			speed = motorbikeItem->Animation.Velocity * cos(motorbikeItem->Pose.Orientation.x);
		else
		...
		if (motorbikeItem->Pose.Position.y >= floorHeight)
		{
			short anglex = MOTORBIKE_SLIP * sin(motorbikeItem->Pose.Orientation.x);
			if (abs(anglex) > 16)
			{
				short anglex2 = MOTORBIKE_SLIP * sin(motorbikeItem->Pose.Orientation.x);
				if (anglex < 0)
					anglex2 = -anglex;
				if (anglex2 > 24)
			...
			short anglez = MOTORBIKE_SLIP * sin(motorbikeItem->Pose.Orientation.z);
			if (abs(anglez) > 32)
			{
				short ang, angabs;
				motorbike->DisableDismount = true;
				...
				if (anglez >= 0)
					ang = motorbikeItem->Pose.Orientation.y + Angle::DegToRad(90.0f);
				else
					ang = motorbikeItem->Pose.Orientation.y - Angle::DegToRad(90.0f);

				angabs = abs(anglez) - 24;
				motorbikeItem->Pose.Position.x += angabs * sin(ang);
				motorbikeItem->Pose.Position.z += angabs * cos(ang);
			}
		}
		...
		if (collide)
		{
			newSpeed = ((motorbikeItem->Pose.Position.z - oldPos.z) * cos(motorbike->MomentumAngle) + (motorbikeItem->Pose.Position.x - oldPos.x) * sin(motorbike->MomentumAngle)) * 256;
	...
	static bool TestMotorbikeDismount(ItemInfo* motorbikeItem, ItemInfo* laraItem)
	{
		...
		short angle = motorbikeItem->Pose.Orientation.y + Angle::DegToRad(90.0f);
			...
			laraItem->Animation.ActiveState == MOTORBIKE_STATE_NONE_6)
		{
				...
				laraItem->Animation.ActiveState == MOTORBIKE_STATE_NONE_6)
			{
				...
				case MOTORBIKE_STATE_IDLE:
					...
					{
						bool dismount = false;
						if ((TrInput & MOTORBIKE_IN_RIGHT) && (TrInput & MOTORBIKE_IN_DISMOUNT))
							dismount = true;
	...
	bool MotorbikeControl(ItemInfo* laraItem, CollisionInfo* coll)
	{
		...
		if (frontMiddle.y >= heightFrontMiddle)
		{
			...
			{
				xRot = atan2(1000, heightFrontMiddle - r1);
				zRot = atan2(MOTORBIKE_SIDE, r2 - frontLeft.y);
			}
			else
			{
				xRot = atan2(MOTORBIKE_FRONT, heightFrontMiddle - motorbikeItem->Pose.Position.y);
				zRot = atan2(MOTORBIKE_SIDE, r2 - frontLeft.y);
			}
		}
		else if (r1 >= ((heightFrontLeft + heightFrontRight) / 2))
		{
			xRot = atan2(MOTORBIKE_FRONT, motorbikeItem->Pose.Position.y - r1);
			zRot = atan2(MOTORBIKE_SIDE, r2 - frontLeft.y);
		}
		else
		{
			xRot = atan2(125, newY - motorbikeItem->Pose.Position.y);
			zRot = atan2(MOTORBIKE_SIDE, r2 - frontLeft.y);
		}
