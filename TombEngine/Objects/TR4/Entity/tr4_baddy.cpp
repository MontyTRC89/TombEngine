...
namespace TEN::Entities::TR4
{
	...
	void InitialiseBaddy(short itemNumber)
	{
		...
		{
			item->SwapMeshBits = MESHSWAPFLAGS_BADDY_GUN;
			item->MeshBits = 0xFF81FFFF;
			item->ItemFlags[2] = BADDY_USE_UZI;
		}
		else
		{
			item->SwapMeshBits = MESHSWAPFLAGS_BADDY_SWORD_NINJA;
	...
	void BaddyControl(short itemNumber)
	{
		...
		else
		{
			...
			case BADDY_STATE_IDLE:
				...
				if (item->SwapMeshBits == MESHSWAPFLAGS_BADDY_SWORD_NINJA &&
				...
				{
					if (item->SwapMeshBits == MESHSWAPFLAGS_BADDY_GUN)
					{
						item->Animation.TargetState = BADDY_STATE_AIM;
						break;
					}

					if (item->SwapMeshBits != MESHSWAPFLAGS_BADDY_SWORD_SIMPLE && item->SwapMeshBits != MESHSWAPFLAGS_BADDY_SWORD_NINJA)
					{
				...
				if (item->SwapMeshBits == MESHSWAPFLAGS_BADDY_GUN && item->ItemFlags[2] < 1)
				{
				...
				if (currentCreature->MonkeySwingAhead)
				{
					...
					{
						if (item->SwapMeshBits == MESHSWAPFLAGS_BADDY_EMPTY)
						{
			...
			case BADDY_STATE_HOLSTER_GUN:
				if (item->Animation.FrameNumber == g_Level.Anims[item->Animation.AnimNumber].frameBase + FRAME_BADDY_HOLSTER_GUN)
					item->SwapMeshBits = MESHSWAPFLAGS_BADDY_EMPTY;
			...
			case BADDY_STATE_DRAW_SWORD:
				...
				if (item->ObjectNumber == ID_BADDY1)
					item->SwapMeshBits = MESHSWAPFLAGS_BADDY_SWORD_SIMPLE;
				else
					item->SwapMeshBits = MESHSWAPFLAGS_BADDY_SWORD_NINJA;
