	...
	void SethaAttack(int itemNumber)
	{
		...
		case 15:
			...
			if (item->ItemFlags[0] >= 96 && item->ItemFlags[0] <= 99)
			{
				...
				auto angles = GetOrientBetweenPoints(pos1.ToVector3(), pos.ToVector3());
			...
			else if (item->ItemFlags[0] >= 122 && item->ItemFlags[0] <= 125)
			{
				...
				auto angles = GetOrientBetweenPoints(pos2.ToVector3(), pos.ToVector3());
		...
		case 12:
				...
				item->ItemFlags[0] >= 112 && item->ItemFlags[0] <= 124)
			{
				if (Wibble & 4)
				{
					...
					angles = GetOrientBetweenPoints(pos2.ToVector3(), pos.ToVector3());
