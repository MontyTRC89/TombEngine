...
void ObeliskControl(short itemNumber)
{
	...
	if (TriggerActive(item))
	{
		...
		if (!(GlobalCounter & 1))
		{
			...
			if (GetRandomControl() & 1)
			{
				if (item->ItemFlags[3] < 256 
					...
					&& !(GlobalCounter & 3))
				{
					SoundEffect(SFX_TEN_ELECTRIC_ONE_SHOT, &item->Pose, 0);
		...
		if (item->ItemFlags[3] >= 256 && item->TriggerFlags == 2)
		{
			...
			SoundEffect(SFX_TEN_ELECTRIC_ARCING_LOOP, &pos, 0);
			...
			if (GlobalCounter & 1)
			{
					...
					abs(pos2.Position.z - LaraItem->Pose.Position.z) < SECTOR(20))
				{
					if (item->ItemFlags[2] != NO_ITEM)
					{
						...
						SoundEffect(SFX_TEN_EXPLOSION1, &item2->Pose, 0);
						SoundEffect(SFX_TEN_EXPLOSION2, &item2->Pose, 0);
					}
