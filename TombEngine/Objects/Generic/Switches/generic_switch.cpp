...
namespace TEN::Entities::Switches
{
	...
	void SwitchCollision(short itemNumber, ItemInfo* laraItem, CollisionInfo* coll)
	{
			...
			laraInfo->Control.IsMoving && laraInfo->Context.InteractedItem == itemNumber)
		{
			...
			SwitchBounds.Bounds.X1 = bounds.X1 - BLOCK(0.25f);
			SwitchBounds.Bounds.X2 = bounds.X2 + BLOCK(0.25f);

			switch (switchItem->TriggerFlags)
			{
				default:
					SwitchPos.z = bounds.Z1 - 128;
					SwitchBounds.Bounds.Z1 = bounds.Z1 - BLOCK(0.2f);
					SwitchBounds.Bounds.Z2 = bounds.Z2;
					break;
				...
				case SWT_SMALL_LEVER:
					...
					SwitchBounds.Bounds.Z1 = bounds.Z1 - BLOCK(0.25f);
					SwitchBounds.Bounds.Z2 = bounds.Z2;
					break;
