...
namespace TEN::Physics
{
	...
	private:
		...
		using CoplanarTriangleMap = std::unordered_map<Plane, std::vector<TriangleVertexIds>>;
		using EdgeCountMap		  = std::unordered_map<EdgeVertexIdPair, int, PairHash>;	   // Key = vertex ID pair defining edge, value = edge count.
		using VertexAdjacencyMap  = std::unordered_map<int, std::unordered_set<int>>;		   // Key = vertex ID, value = adjacent vertex ID.
