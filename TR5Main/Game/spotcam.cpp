...
void CalculateSpotCameras()
{
		...
		|| !(TrInput & IN_LOOK))
	{
		...
		if (CheckTrigger)
		{
			...
			if (CurrentLevel != 0)
			{
				TestTriggers(Camera.pos.x, Camera.pos.y, Camera.pos.z, Camera.pos.roomNumber, 1, 0);
			}
			else
			{
				TestTriggers(Camera.pos.x, Camera.pos.y, Camera.pos.z, Camera.pos.roomNumber, 0, 0);
				TestTriggers(Camera.pos.x, Camera.pos.y, Camera.pos.z, Camera.pos.roomNumber, 1, 0);
			}
			Camera.type = oldType;
			CheckTrigger = false;
