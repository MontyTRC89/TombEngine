...
void lara_col_slopeclimb(ITEM_INFO* item, CollisionInfo* coll)
{
	...
	PHD_VECTOR now = { item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z };
	...
	PHD_VECTOR up = { item->Pose.Position.x - offset.x, item->Pose.Position.y - CLICK(1), item->Pose.Position.z - offset.z };
	PHD_VECTOR down = { item->Pose.Position.x + offset.x, item->Pose.Position.y + CLICK(1), item->Pose.Position.z + offset.z };
...
void lara_col_slopeshimmy(ITEM_INFO* item, CollisionInfo* coll)
{
	...
	PHD_VECTOR shimmy = { item->Pose.Position.x, item->Pose.Position.y, item->Pose.Position.z };
