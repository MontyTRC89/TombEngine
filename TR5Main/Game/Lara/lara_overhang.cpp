...
void SlopeReachExtra(ITEM_INFO* item, CollisionInfo* coll)
{
	...
	if (probeNow.BottomBlock->Flags.Monkeyswing && ceilDist <= CLICK(3.5f))
	{
		...
		if (abs(probeNow.CeilingTilt.x) > 2 || abs(probeNow.CeilingTilt.y) > 2 || bridge >= 0)
		{
			...
			{
				if (abs(OrientDelta(item->Pose.Orientation.y, slopeData.GoalOrient)) < EulerAngle::DegToRad(33.75f))
					disableGrab = false;
			}
...
void SlopeClimbExtra(ITEM_INFO* item, CollisionInfo* coll)
{
	...
	if (item->Animation.AnimNumber == LA_LADDER_IDLE)
	{
		if (TrInput & IN_FORWARD)
		{
			...
			{
				short facing = item->Pose.Orientation.y + EulerAngle::DegToRad(45.0f);
...
void SlopeMonkeyExtra(ITEM_INFO* item, CollisionInfo* coll)
{
	...
	if (TrInput & IN_FORWARD) // Monkey to slope transitions.
	{
		...
		{
			if (abs(OrientDelta(slopeData.GoalOrient, item->Pose.Orientation.y)) <= EulerAngle::DegToRad(30.0f) &&
		...
		if (lara->Control.CanMonkeySwing)
		{
			...
			if (probe.BottomBlock->Flags.ClimbPossible(GetClimbDirection(item->Pose.Orientation.y + EulerAngle::DegToRad(180.0f))) &&
