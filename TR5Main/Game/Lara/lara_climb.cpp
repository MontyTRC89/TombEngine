...
void lara_col_climbdown(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	int shiftLeft = 0;
	int shiftRight = 0;
	int resultRight = LaraTestClimbPos(item, coll->Settings.Radius, coll->Settings.Radius + 120, -512, 512, &shiftRight);
	int resultLeft = LaraTestClimbPos(item, coll->Settings.Radius, -(coll->Settings.Radius + 120), -512, 512, &shiftLeft);
...
void lara_col_climbing(ITEM_INFO* item, COLL_INFO* coll)
{
	if (!LaraCheckForLetGo(item, coll) && item->animNumber == LA_LADDER_UP)
	{
		...
		resultRight = LaraTestClimbUpPos(item, coll->Settings.Radius, coll->Settings.Radius + 120, &shiftRight, &ledgeRight);
		resultLeft = LaraTestClimbUpPos(item, coll->Settings.Radius, -(coll->Settings.Radius + 120), &shiftLeft, &ledgeLeft);
...
void lara_col_climbright(ITEM_INFO* item, COLL_INFO* coll)
{
	if (!LaraCheckForLetGo(item, coll))
	{
		...
		LaraDoClimbLeftRight(item, coll, LaraTestClimbPos(item, coll->Settings.Radius, coll->Settings.Radius + 120, -512, 512, &shift), shift);
	}
}
...
void lara_col_climbstnc(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	if (!(TrInput & IN_FORWARD))
	{
		...
		resultRight = LaraTestClimbPos(item, coll->Settings.Radius, coll->Settings.Radius + 120, -512, 512, &ledgeRight);
		resultLeft = LaraTestClimbPos(item, coll->Settings.Radius, -120 - coll->Settings.Radius, -512, 512, &ledgeLeft);
	...
	else if (item->goalAnimState != LS_GRABBING)
	{
		item->goalAnimState = LS_LADDER_IDLE;
		resultRight = LaraTestClimbUpPos(item, coll->Settings.Radius, coll->Settings.Radius + 120, &shiftRight, &ledgeRight);
		resultLeft = LaraTestClimbUpPos(item, coll->Settings.Radius, -120 - coll->Settings.Radius, &shiftLeft, &ledgeLeft);
...
void LaraDoClimbLeftRight(ITEM_INFO* item, COLL_INFO* coll, int result, int shift)
{
	...
	if (coll->Settings.OldAnimState != LS_LADDER_IDLE)
	{	
		item->animNumber = LA_LADDER_IDLE;
		item->frameNumber = g_Level.Anims[item->animNumber].frameBase;
		return;
...
int LaraClimbRightCornerTest(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	if (GetClimbFlags(x, item->pos.yPos, z, item->roomNumber) & (short)LeftExtRightIntTab[angle])
	{
		...
		result = LaraTestClimbPos(item, coll->Settings.Radius, coll->Settings.Radius + 120, -512, 512, &shift);
	}
	...
	if (!result)
	{
		...
		if (GetClimbFlags(newX, item->pos.yPos, newZ, item->roomNumber) & (short)LeftIntRightExtTab[angle])
		{
			...
			item->pos.yRot -= ANGLE(90);
			Lara.moveAngle = item->pos.yRot;
			result = LaraTestClimbPos(item, coll->Settings.Radius, coll->Settings.Radius + 120, -512, 512, &shift) != 0;
		}
	}
	else
	{
...
int LaraClimbLeftCornerTest(ITEM_INFO* item, COLL_INFO* coll)
{
	...
	if (!result)
	{
		...
		if (GetClimbFlags(newX, item->pos.yPos, newZ, item->roomNumber) & (short)LeftExtRightIntTab[angle])
		{
			...
			item->pos.yRot += ANGLE(90);
			Lara.moveAngle = item->pos.yRot;
			item->itemFlags[3] = LaraTestClimbPos(item, coll->Settings.Radius, -coll->Settings.Radius - 120, -512, 512, &shift);
