...
void lara_as_climb_end(ITEM_INFO* item, CollisionInfo* coll)
{
	...
	Camera.targetAngle = EulerAngle::DegToRad(-45.0f);
}
...
int GetClimbFlags(FLOOR_INFO* floor)
{
	...
	if (floor->Flags.ClimbEast)
		result |= (int)CLIMB_DIRECTION::East;

	if (floor->Flags.ClimbWest)
		result |= (int)CLIMB_DIRECTION::West;

	if (floor->Flags.ClimbNorth)
		result |= (int)CLIMB_DIRECTION::North;

	if (floor->Flags.ClimbSouth)
		result |= (int)CLIMB_DIRECTION::South;
...
void LaraDoClimbLeftRight(ITEM_INFO* item, CollisionInfo* coll, int result, int shift)
{
	...
	if (TrInput & IN_LEFT)
	{
		...
		int dx = int(sin(item->Pose.Orientation.GetY() - EulerAngle::DegToRad(90.0f)) * 10);
		int dz = int(cos(item->Pose.Orientation.GetY() - EulerAngle::DegToRad(90.0f)) * 10);
...
int LaraClimbRightCornerTest(ITEM_INFO* item, CollisionInfo* coll)
{
	...
	if (GetClimbFlags(x, item->Pose.Position.y, z, item->RoomNumber) & (int)LeftExtRightIntTab[quadrant])
	{
		...
		lara->NextCornerPos.Orientation.SetY(item->Pose.Orientation.GetY() + EulerAngle::DegToRad(90.0f));
	...
	if (!result)
	{
		...
		if (GetClimbFlags(x, item->Pose.Position.y, z, item->RoomNumber) & (int)LeftIntRightExtTab[quadrant])
		{
			lara->NextCornerPos.Position.x = item->Pose.Position.x = x;
			lara->NextCornerPos.Position.y = item->Pose.Position.y;
			lara->NextCornerPos.Position.z = item->Pose.Position.z = z;
...
int LaraClimbLeftCornerTest(ITEM_INFO* item, CollisionInfo* coll)
{
	...
	{
		item->Pose = oldPose;
		lara->Control.MoveAngle = oldOrient;
