...
void SetLaraCornerAnimation(ITEM_INFO* item, CollisionInfo* coll, bool flip)
{
	...
	if (item->HitPoints <= 0)
	{
		...
		item->Pose.Orientation.y += lara->NextCornerPos.yRot / 2;
		item->Animation.Velocity = 2;
		item->Animation.VerticalVelocity = 1;
		item->Animation.Airborne = true;
		lara->Control.HandStatus = HandStatus::Free;
	...
	if (flip)
	{
		...
		coll->Setup.OldPosition.x = item->Pose.Position.x = lara->NextCornerPos.xPos;
		coll->Setup.OldPosition.y = item->Pose.Position.y = lara->NextCornerPos.yPos;
		coll->Setup.OldPosition.z = item->Pose.Position.z = lara->NextCornerPos.zPos;
		item->Pose.Orientation.y = lara->NextCornerPos.yRot;
	}
}
