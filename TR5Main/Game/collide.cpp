...
COLL_RESULT GetCollisionResult(FLOOR_INFO* floor, int x, int y, int z)
{
	...
	result.Position.Bridge = result.BottomBlock->InsideBridge(x, z, result.Position.Floor + 1, false, false) >= 0;
	result.Position.Slope = !result.Position.Bridge && ((abs(tilts.first)) > 2 || (abs(tilts.second)) > 2);
