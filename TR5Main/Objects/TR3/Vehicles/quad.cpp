...
#define QUAD_UNDO_TURN			Angle::DegToRad(2.0f)
#define QUAD_TURN_RATE			(Angle::DegToRad(0.5f) + QUAD_UNDO_TURN)
#define QUAD_TURN_MAX			Angle::DegToRad(5.0f)
#define QUAD_DRIFT_TURN_RATE	(Angle::DegToRad(0.75f) + QUAD_UNDO_TURN)
#define QUAD_DRIFT_TURN_MAX		Angle::DegToRad(8.0f)

#define MIN_MOMENTUM_TURN Angle::DegToRad(3.0f)
#define MAX_MOMENTUM_TURN Angle::DegToRad(1.5f)
#define QUAD_MAX_MOM_TURN Angle::DegToRad(150.0f)
...
void InitialiseQuadBike(short itemNumber)
{
	...
	quad->TurnRate = 0;
	quad->MomentumAngle = quadItem->Pose.Orientation.GetY();
...
static int CanQuadbikeGetOff(int direction)
{
	...
	if (direction < 0)
		angle = item->Pose.Orientation.GetY() - Angle::DegToRad(90.0f);
	else
		angle = item->Pose.Orientation.GetY() + Angle::DegToRad(90.0f);
...
static bool QuadCheckGetOff(ItemInfo* laraItem, ItemInfo* quadItem)
{
		...
		TestLastFrame(laraItem))
	{
		...
		laraItem->Pose.Position.x -= DISMOUNT_DISTANCE * sin(laraItem->Pose.Orientation.GetY());
		laraItem->Pose.Position.z -= DISMOUNT_DISTANCE * cos(laraItem->Pose.Orientation.GetY());
...
static int GetOnQuadBike(ItemInfo* laraItem, ItemInfo* quadItem, CollisionInfo* coll)
{
	...
	else
	{
		...
		angle = Angle::Normalize(angle - quadItem->Pose.Orientation.GetY());

		if ((angle > Angle::DegToRad(-45.0f)) && (angle < Angle::DegToRad(135.0f)))
		{
			float tempAngle = Angle::Normalize(laraItem->Pose.Orientation.GetY() - quadItem->Pose.Orientation.GetY());
			if (tempAngle > Angle::DegToRad(45.0f) && tempAngle < Angle::DegToRad(135.0f))
				return true;
			else
				return false;
		}
...
static int QuadDynamics(ItemInfo* laraItem, ItemInfo* quadItem)
{
	...
	if (quadItem->Pose.Position.y > (quadItem->Floor - CLICK(1)))
	{
		...
		quadItem->Pose.Orientation.SetY(quadItem->Pose.Orientation.GetY() + quad->TurnRate + quad->ExtraRotation);
		...
		float rot = quadItem->Pose.Orientation.GetY() - quad->MomentumAngle;
		if (rot < -MAX_MOMENTUM_TURN)
		{
			if (rot < -QUAD_MAX_MOM_TURN)
			{
				rot = -QUAD_MAX_MOM_TURN;
				quad->MomentumAngle = quadItem->Pose.Orientation.GetY() - rot;
			}
			else
	...
	if (quadItem->Pose.Position.y >= probe.Position.Floor)
		speed = quadItem->Animation.Velocity * cos(quadItem->Pose.Orientation.GetX());
	else
	...
	int slip = QUAD_SLIP * sin(quadItem->Pose.Orientation.GetX());
	if (abs(slip) > QUAD_SLIP / 2)
	{
		if (slip > 0)
			...
			slip += 10;
		quadItem->Pose.Position.z -= slip * cos(quadItem->Pose.Orientation.GetY());
	...
	if (abs(slip) > QUAD_SLIP_SIDE / 2)
	{
		...
		quadItem->Pose.Position.x += slip * cos(quadItem->Pose.Orientation.GetY());
	}
...
static int QuadUserControl(ItemInfo* quadItem, int height, int* pitch)
{
	...
	if (quadItem->Pose.Position.y >= (height - CLICK(1)))
	{
		...
		else if (TrInput & QUAD_IN_ACCELERATE)
		{
			...
			quad->Velocity -= abs(quadItem->Pose.Orientation.GetY() - quad->MomentumAngle) / 64;
		}
...
void QuadBikeCollision(short itemNumber, ItemInfo* laraItem, CollisionInfo* coll)
{
	...
	if (GetOnQuadBike(laraItem, &g_Level.Items[itemNumber], coll))
	{
		...
		float angle = atan2(quadItem->Pose.Position.z - laraItem->Pose.Position.z, quadItem->Pose.Position.x - laraItem->Pose.Position.x);
		angle -= quadItem->Pose.Orientation.GetY();
		...
		laraItem->Pose.Orientation.SetY(quadItem->Pose.Orientation.GetY());
...
bool QuadBikeControl(ItemInfo* laraItem, CollisionInfo* coll)
{
		...
		laraItem->Animation.ActiveState != QUAD_STATE_DISMOUNT_LEFT)
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			angle = quadItem->Pose.Orientation.GetY() + ((i == 0) ? 0x9000 : 0x7000);
