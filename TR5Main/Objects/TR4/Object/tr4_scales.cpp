...
void ScalesCollision(short itemNumber, ItemInfo* laraItem, CollisionInfo* coll)
{
	...
	if (TestBoundsCollide(item, laraItem, LARA_RADIUS))
	{
		...
		else
		{
			...
			item->Pose.Orientation.SetY(Angle::ShrtToRad((Angle::RadToShrt(laraItem->Pose.Orientation.GetY() + Angle::DegToRad(45.0f)) & Angle::DegToShrt(270.0f))));
			...
			if (TestLaraPosition(&ScalesBounds, item, laraItem))
			{
				...
				item->Pose.Orientation.SetY(rotY);
			}
