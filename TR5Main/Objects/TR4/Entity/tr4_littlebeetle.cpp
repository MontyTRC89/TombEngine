...
void LittleBeetleControl(short itemNumber)
{
	...
	if (item->triggerFlags)
	{
		if (!item->itemFlags[2] || !(GetRandomControl() & 0xF))
		{
			...
			short beetleNum = GetNextLittleBeetle();
			if (beetleNum != NO_ITEM)
			{
				BEETLE_INFO* beetle = &LittleBeetles[beetleNum];
...
{
	short result = NextLittleBeetle;
	BEETLE_INFO* beetle = &LittleBeetles[NextLittleBeetle];
	...
	NextLittleBeetle = (result + 1) & (NUM_LITTLE_BETTLES - 1);
