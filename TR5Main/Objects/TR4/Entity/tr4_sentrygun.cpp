	...
	void SentryGunControl(short itemNum)
	{
		...
		if (item->MeshBits & 0x40)
		{
			...
			if (item->TriggerFlags == 0)
			{
				...
				float deltaAngle = Angle::ShortestAngle(AI.angle, creature->JointRotation[0]);
				...
				if (Targetable(item, &AI))
				{
					if (AI.distance < pow(SECTOR(9), 2))
					{
						...
						if (deltaAngle <= Angle::DegToRad(10.0f))
						{
							if (deltaAngle < Angle::DegToRad(-10.0f))
								deltaAngle = Angle::DegToRad(-10.0f);
						}
						else
							deltaAngle = Angle::DegToRad(10.0f);
				...
				if (creature->JointRotation[2] > Angle::DegToRad(90.0f) ||
					creature->JointRotation[2] < Angle::DegToRad(-90.0f))
				{
			...
			else
			{
				...
				CreatureJoint(item, 1, Angle::DegToRad(45.0f));
				CreatureJoint(item, 2, (GetRandomControl() & 0x3FFF) - Angle::DegToRad(45.0f));
			}
		}
		else
		{
