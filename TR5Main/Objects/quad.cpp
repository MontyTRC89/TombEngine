...
#define QUAD_UNDO_TURN	(ONE_DEGREE*2)
#define QUAD_TURN			((ONE_DEGREE/2) + QUAD_UNDO_TURN)
#define QUAD_MAX_TURN		(ONE_DEGREE*5)
#define QUAD_HTURN		(((ONE_DEGREE*3)/4) + QUAD_UNDO_TURN)
#define QUAD_MAX_HTURN		(ONE_DEGREE*8)

#define MIN_MOMENTUM_TURN	(ONE_DEGREE*3)
#define MAX_MOMENTUM_TURN	(ONE_DEGREE+(ONE_DEGREE/2))
#define QUAD_MAX_MOM_TURN (ONE_DEGREE*150)
...
void __cdecl QuadbikeExplode(ITEM_INFO* item)
{
	...
	ExplodingDeath(LaraVehicle, 0xfffffffe, 1);
	KillItem(LaraVehicle);
...
__int32 __cdecl CanGetOff(__int32 direction)
{
	...
	ITEM_INFO* item = &Items[LaraVehicle];
...
__int32 __cdecl QuadBikeControl()
{
	...
	if (!(quad->flags & QUAD_FLAGS_DEAD))
	{
		...
		{
			ItemNewRoom(LaraVehicle, roomNumber);
